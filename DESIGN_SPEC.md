...
- **Cold-boot splash:** a static HTML splash (logo + thin top bar + "Loading JotJSON..." label) is inlined into `<app-root>` in `index.html` so it renders before any JS executes. Angular's `bootstrapApplication` removes it automatically on first render. The splash respects `prefers-color-scheme` and `prefers-reduced-motion`. Pre-Angular-bootstrap strings here (the "Loading JotJSON..." label) are an i18n exception by necessity (the i18n pipeline runs after bootstrap).
- **Route progress bar:** while any router navigation is in flight (resolver, lazy chunk, redirect), an indeterminate top-of-viewport bar (8px, primary cyan with glow) appears - matching the splash bar visually so the handoff at first NavigationEnd is continuous. The bar is decorative (`aria-hidden`); the route change itself is what assistive tech announces. Tracking is set-based on router event ids so resolver-redirect-to-/404 sequences (where the original navigation cancels and a new one starts) keep the bar visible without flicker.
