...
- **JSON Input Panel** (left or top, depending on layout preference)
  ...
  - **Sort keys** - alphabetizes object keys recursively across the whole document while preserving the surrounding text byte-for-byte: number precision, escape forms (e.g., `"\u0041"`), trailing commas, and comments at every level - leading/trailing document comments, value-internal comments, and inter-property comments inside each sorted object body - all survive sort. Inter-property comments are attributed to their neighbouring property via source-position rules (gap-split on first non-comment newline). Known limits: a blank line between properties may visually drift on sort; a `// tail` comment placed after the source-last property's trailing comma may migrate between properties; leading-comma source layouts produce stylistically mixed output. The current document's whitespace style is preserved (Sort no longer re-pretty-prints compact input). Both a 30-second Undo snackbar and the editor's built-in undo (Ctrl+Z) recover the pre-Sort content.
...
- **Tree View Panel** (right or bottom, depending on layout preference)
  ...
  - **Sort keys.** Two entry points: the toolbar Sort button alphabetizes object keys recursively across the whole document, while the row-level `Sort keys` context-menu item on object nodes reorders only that object's direct keys in place. Both paths use the same byte-splicing primitive (`partitionObjectContent` + `serializeObjectPartition` in `sort-json-patcher.ts`) so they share the same fidelity contract: numbers, escape forms, trailing commas, surrounding comments, and inter-property comments inside the sorted object body all survive. Inter-property comments are attributed to their neighbouring property via source-position rules; blank lines, trailing-comma-with-line-comment, and leading-comma source styles may visually drift but remain valid JSONC. The row-level action is hidden for arrays, primitives, empty objects, and single-key objects because it would be a no-op.
  - **Inspect string value dialog for string leaves.** String values whose JSON-escaped form is hard to read at a glance (anything containing a newline, carriage return, tab, embedded `"`, or `\`, OR any string longer than 256 characters) get a small `Inspect string value` pill, plus a matching `Inspect string value` entry in the row's context menu. Clicking either affordance opens a dedicated dialog that renders the raw string with line numbers, a Copy button, and a larger monospace font - mobile-friendly for long payloads. When the same string is also extractable, this dialog hosts the primary Extract action; the row-level Extract pill is hidden in that combined case so the destructive step no longer sits beside the non-destructive viewer pill, while the context-menu Extract entry remains available for direct action. The pill is stateless: row height stays uniform whether the dialog is open or not. Opening the dialog does NOT mutate the underlying value, change copy semantics (the dialog's Copy button writes the raw string), or affect tree search (which matches the canonical JSON-escaped form in substring mode; regex mode matches the raw string value with multi-line `^/$` anchors - see the Match semantics for tree search note in §Search highlight below). Inline string rendering in the tree is always the JSON-escaped single-line form; multi-line `pre-wrap` rendering is reserved for the dialog viewer. Each open logs `tree.decoded.viewerOpened` with `source` (`rowButton` / `contextMenu`), `reason` (`escape` for character-driven matches; `long` for length-only matches), `pathDepth` (bucketed), `lineCountBucket`, and `manglingKind` (`none` / `httpFraming`) - never the string contents or raw paths. See milestone M7v for the safety overhaul (dialog-hosted Extract, native + snackbar Undo). When the heuristic detects a string whose `??` count exceeds its count of preserved line breaks (any of `\r\n`, `\n`, `\r`) - a permissive, intentionally count-based rule that captures the Microsoft/Azure dependent-service trace pattern of CRLF-to-`??` lossy transcoding without requiring an HTTP-shaped grammar match - a `MatSlideToggle` appears in a sub-header strip labeled `Show "??" as line breaks`. The toggle defaults off (raw); flipping it on re-renders the value via a decoder that substitutes `??` -> CRLF (`\r\n` between every `??`-separated segment, `\r\n\r\n` at any `????` header/body boundary). When the value contains a `????` separator the decoder is **prefix-only**: only the section before the separator is rewritten, and the body is preserved verbatim so any `??` inside the body (URLs, base64 fragments, recursively mangled payloads) survives. When no `????` separator is present the decoder is a straight global replace `??` -> `\r\n`; the user previews the result via the toggle before deciding whether to commit it via Apply. The decoder writes CRLF characters into the *string value*; the JSON source remains single-line because string literals encode those characters as the `\r\n` escape sequence. The toggle is dialog-local and not persisted. The dialog's Copy button is labelled `Copy value` (reusing the shipped row-level i18n IDs `@@tree.contextMenu.copyValue` / `@@tree.contextMenu.copy.success.value` / `@@tree.contextMenu.copy.failed.value` / `@@tree.contextMenu.copy.unsupported`). Copy is **WYSIWYG**: it writes whatever the user is currently looking at - the raw `??`-mangled string when the toggle is off (the default), the CRLF-decoded form when the toggle is on. This makes Copy a non-mutating mirror of Apply for users who want to paste the decoded text elsewhere without rewriting the JSON source. The row-level Copy entry in the tree still copies the canonical raw string verbatim, so the dialog-Copy WYSIWYG behaviour cannot leak into non-dialog code paths. In decoded mode an additional **Apply** button (`mat-stroked-button color="primary"`, labelled "Replace ?? with line breaks") appears between Extract and Copy; clicking it closes the dialog with an `applyDecoded` intent and asks `HomeComponent` to rewrite the underlying JSON string literal in place. The mutation uses the patcher at `src/app/features/home/decoded-apply-patcher.ts`, which (a) re-parses the document with `jsonc-parser`, (b) locates the string node by path, (c) computes `decodeLossyMangling(rawValue, manglingKind)` with `httpFraming` emitting CRLF (`\r\n` header separator, `\r\n\r\n` body separator - spec-canonical HTTP framing chosen so the rewritten string round-trips cleanly through HTTP tooling), (d) replaces the literal via `editor.applyEdit` inside the named undo group `jotjson-decoded-apply`, and (e) installs a `pendingReplaceUndo` of kind `decoded.apply` paired with an 8s assertive snackbar reading "Replaced \"??\" markers with line breaks in the document." and carrying an Undo button. The undo lifecycle matches Extract: native `Ctrl+Z` collapses the change atomically via the named undo group, and the snackbar Undo restores `priorText`; both paths emit `home.decodedApply.undo` with `source: 'snackbar' | 'ctrlZ'`. Apply is gated on `manglingActive() && decoded()` so the user has previewed the decoded form before committing. Tab order is body content -> toggle -> Extract -> Apply -> Copy -> Close; flipping the toggle announces the new state via `LiveAnnouncer` (`@angular/cdk/a11y`) and emits `tree.decoded.manglingToggle` with `to: 'raw' | 'decoded'`. Apply emits `tree.decoded.apply` (dialog click), `tree.decoded.apply.staleClose` (tree-side re-validation failed), `home.decodedApply.applied` (post-patch success, props `{ source: 'decodedDialog', manglingKind }`), `home.decodedApply.applyFailed` (warn, props `{ reason }`), and `home.decodedApply.undo` (props `{ source }`) - all carry the closed-enum `manglingKind` where applicable so funnel analysis can slice by kind. The heuristic + decoder live in `src/app/core/text/lossy-mangling.ts` under a discriminated union `LossyManglingKind = 'none' | 'httpFraming'` that is forward-compatible with future kinds (`stackTrace`, `pem`, ...) without breaking existing call sites. The `'httpFraming'` kind name is retained for telemetry stability and reflects the decoder's output shape (HTTP-canonical CRLF framing); the v1.3 detection rule has broadened from ">=3 header-shape `??Name: value` matches" to the count-based rule above, so the kind name is more about the decoder's output than the detector's input.
...
7. **Polish & launch** - Each of these lands as its own step/commit:
   ...
   - ~~**M7r**: Title suggester (wand button) - heuristic suggestions for the document title. A small wand icon button between the title input and the state pill (signed-in users only) opens a menu of 2-7 candidate titles inferred from the current document. Computed lazily on click via a registry of pure strategy functions in `src/app/core/title-suggester/`, ordered by confidence and deduplicated case-insensitively, against the already-memoized parsed value plus the most recent uploaded/dropped file's name. Strategies cover known formats (`package.json`, Kubernetes, OpenAPI/Swagger, JSON Schema, GeoJSON, ARM, `tsconfig`, GitHub Actions workflows, Postman), HAL self-links, common identifier fields, type discriminators, top-level keys, descriptions, generic shape descriptions, and last-resort `firstChars` / `Untitled` fallbacks. A post-dedupe synthetic floor (`Untitled - YYYY-MM-DD` then `Untitled (n)`) guarantees at least 2 menu entries. The candidate's literal text is never logged in telemetry; only the strategy `source` and total candidate count.~~ (done)
    ...
    - **M7w**: JSON sort - a toolbar Sort button alphabetizes object keys recursively across the whole document, and a row-level `Sort keys` context-menu item on object nodes sorts only that object's direct keys in place. Both paths use the same byte-splicing primitive (`patchSortKeysDeep` walks the AST once and rebuilds each object body bottom-up from the AST; `patchSortKeysAtPath` patches a single targeted object). Numbers, escape forms, trailing commas, surrounding comments, and inter-property comments inside each sorted object body are all preserved; attribution is source-position based (gap-split on first non-comment newline), with documented visual drift limits for blank lines, trailing-comma + same-line line comments, and leading-comma source layouts. Mode is auto-derived from the post-Sort content via the existing detectMode effect (no forced mode flip).
