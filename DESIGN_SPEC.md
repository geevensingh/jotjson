...
- **Tree View Panel** (right or bottom, depending on layout preference)
  ...
  - **Per-row context menu** - right-click on any tree row, or click the row's kebab button (always visible at the right edge, sibling to the copy-path pill, low-contrast styling that brightens on hover), opens a single shared menu of row-level actions. Items adapt to the row's kind and the current expansion state, and are hidden when not applicable:
    ...
    - **Copy path** - same as the existing copy-path pill; respects `treePathRoot`.
    ...
    - **Expand to depth +1..+5 from here** - "snap to exactly +N" semantics: containers at relative depth `< N` are expanded and any expanded containers at relative depth `>= N` are collapsed. Each entry is hidden when the subtree is already exactly at that depth.
  ...
  - **Double-click a row** copies the row's value to the clipboard with the same extraction semantics as the menu's **Copy value** action (raw text for primitives, pretty-printed JSON for containers). The dblclick path excludes the kebab pill, copy-path pill, and twisty toggle the same way single-click selection does, so clicking those buttons twice never triggers an unintended value copy. Emits the `tree.row.doubleClickCopyValue` telemetry event.
