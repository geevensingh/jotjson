...
- **JSON Input Panel** (left or top, depending on layout preference)
  ...
  - **Sort keys** - rewrites the current editor text as pretty-printed JSON with object keys alphabetized recursively across the whole document. Like Minify, this serialization removes JSONC comments. Both a 30-second Undo snackbar and the editor's built-in undo (Ctrl+Z) recover the pre-Sort content.
