...
- **Tree View Panel** (right or bottom, depending on layout preference)
  ...
  - **Ancestor breadcrumb** - a single-line breadcrumb sits between the expansion controls and the tree, showing the parent chain of the currently-selected row as chevron-separated chips (e.g., `Root > users > [0]`). The chips are clickable buttons; clicking one re-selects that ancestor (which expands the path and scrolls it into view via the same `selectByPathString` flow as editor->tree sync). The first chip is always the literal label `Root` (decoupled from `treePathRoot` clipboard formatting - this is navigation, not paste output) and points at `$`. The currently-selected node itself is **not** in the breadcrumb (the user described it as "parent nodes"); a root selection therefore shows just the single Root chip. With no selection, the breadcrumb shows a muted `No current selection` placeholder (`aria-live="polite"`) so the slot does not look empty. When a path has more than five ancestors, the middle chips collapse into an overflow chip (`...`) that opens a `mat-menu` listing the hidden ancestors in tree order. Per-chip width is capped (`max-width: 14ch`) with text-overflow ellipsis and a `title` tooltip to keep one long key from blowing out the row. Each click emits a `tree.breadcrumb.click` info-level telemetry event with the 0-based depth only - the canonical path string (which may contain user content) is **not** logged. The breadcrumb is visible in `embeddedMode` (e.g., the rule-editor live preview) for orientation.
