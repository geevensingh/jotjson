...
- **Shipped in v1:**
  ...
  - **Deployment cache headers**: `staticwebapp.config.json` sets `Cache-Control: no-cache, must-revalidate` on four shell / manifest paths so browsers revalidate them on every load, shrinking the window where stale references can collide with a new build: `/index.html` and `/404/index.html` (prerendered home and 404 shells - see "Server-platform safety" in AGENTS.md), `/shell.html` (the SPA navigation fallback served on every non-prerendered route), and `/ngsw.json` (the service-worker hash manifest). `scripts/check-swa-config.mjs` (lint chain) asserts all four rules are present with the canonical value, and that no wildcard route precedes them (route-order shadowing would silently re-open the stale-shell class without deleting the rules themselves).
