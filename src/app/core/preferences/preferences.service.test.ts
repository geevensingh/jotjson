...
class UserApiServiceStub {
  getMe = vi.fn().mockReturnValue(of<UserWithEtag | null>(null));
  seed = vi.fn().mockImplementation((prefs: UserPreferences) =>
    of<UserWithEtag>({
      user: {
        id: 'u-1',
        displayName: 'Test',
...
describe('PreferencesService', () => {
  ...
  it('drops unknown keys when hydrating from a remote response and folds legacy defaultRuleSetIds', async () => {
    ...
    const svc = TestBed.inject(PreferencesService);
    api.getMe.mockReturnValue(of({ user, etag: '"1"' }));
    auth.signInAs('u-1');
    TestBed.flushEffects();
  ...
  it('backfills partial nested treeHighlightColors from the server', async () => {
    ...
    const svc = TestBed.inject(PreferencesService);
    api.getMe.mockReturnValue(of(makeUserResponse(partial)));
    auth.signInAs('u-1');
    TestBed.flushEffects();
    await svc.__waitForSync();
    const colors = svc.prefs().treeHighlightColors;
  ...
  it('applies treeDateAnnotationFriendlyForms boolean patches', () => {
    ...
    expect(svc.prefs().treeDateAnnotationFriendlyForms).toBe(false);

    svc.update({ treeDateAnnotationFriendlyForms: true });
    expect(svc.prefs().treeDateAnnotationFriendlyForms).toBe(true);
  });
  ...
  describe('pref.changed telemetry', () => {
    ...
    it('sign-in hydration emits init-sourced events for remote preference diffs', async () => {
      const svc = injectPrefs();
      api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'dark', editorFontSize: 18 })));
  ...
  describe('theme.applied telemetry', () => {
    ...
    function themeAppliedCalls(): Array<{
      ...
      return logger.event.mock.calls.reduce<
        Array<{
          effective: 'dark' | 'light';
          pref: UserPreferences['theme'];
          source: string;
    ...
    it('init-source emit on sign-in hydration that moves effective theme', async () => {
      ...
      const svc = TestBed.inject(PreferencesService);
      api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'dark' })));
      logger.event.mockClear();
    ...
    it('applyPrefs reads cached effectiveTheme to avoid redundant matchMedia (regression guard for #374)', () => {
      ...
      expect(matchMediaSpy.mock.calls.length).toBe(1);
    });
  });
  ...
  describe('runtime --highlight-* projection on document.body', () => {
    ...
    it('sign-out projects default colors back onto body', async () => {
      ...
      api.getMe.mockReturnValue(of(makeUserResponse(customized)));
      auth.signInAs('u-1');
      TestBed.flushEffects();
      await svc.__waitForSync();
      TestBed.flushEffects();
    ...
    it('coalesces rapid highlight-color updates into a single PUT', async () => {
      ...
      try {
        const svc = TestBed.inject(PreferencesService);
        api.getMe.mockReturnValue(of(makeUserResponse()));
        auth.signInAs('u-1');
        TestBed.flushEffects();
        await svc.__waitForSync();
        api.putPreferences.mockClear();
  ...
  describe('sync lifecycle', () => {
    it('replaces local prefs with the server copy when the user doc exists (remote wins)', async () => {
      ...
      TestBed.flushEffects();
      api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'light', editorFontSize: 16 })));
      auth.signInAs('u-1');
      TestBed.flushEffects();
    ...
    it('seeds the server from local prefs on first-ever sign-in (404)', async () => {
      ...
      svc.update({ theme: 'dark' });
      TestBed.flushEffects();
      api.getMe.mockReturnValue(of(null));
      auth.signInAs('u-1');
      TestBed.flushEffects();
      const end = await svc.__waitForSync();
      expect(end).toBe('synced');
    ...
    it('marks syncState=error when GET /api/me fails', async () => {
      const svc = TestBed.inject(PreferencesService);
      api.getMe.mockReturnValue(throwError(() => new Error('network')));
      auth.signInAs('u-1');
      TestBed.flushEffects();
      const end = await svc.__waitForSync();
    ...
    it('clears previous user prefs from localStorage and resets on sign-out', async () => {
      const svc = TestBed.inject(PreferencesService);
      api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'light', editorFontSize: 22 })));
      auth.signInAs('u-1');
      TestBed.flushEffects();
      await svc.__waitForSync();
      expect(svc.prefs().theme).toBe('light');
    ...
    it('ignores in-flight hydration results when the user changes', async () => {
      ...
      api.getMe.mockReturnValue(slow.asObservable());
      auth.signInAs('u-1');
      TestBed.flushEffects();
    ...
    it('threads the latest etag from getMe -> putPreferences', async () => {
      ...
      try {
        const svc = TestBed.inject(PreferencesService);
        api.getMe.mockReturnValue(of(makeUserResponse({}, '"7"')));
        auth.signInAs('u-1');
        TestBed.flushEffects();
        await svc.__waitForSync();
        svc.update({ theme: 'dark' });
    ...
    it('threads the latest etag from a successful PUT into the next PUT', async () => {
      ...
      try {
        const svc = TestBed.inject(PreferencesService);
        api.getMe.mockReturnValue(of(makeUserResponse({}, '"1"')));
        // Each successive PUT returns an incrementing etag.
        let nextEtag = 2;
        api.putPreferences.mockImplementation((p: UserPreferences) =>
        ...
        TestBed.flushEffects();
        vi.advanceTimersByTime(600);
        expect(api.putPreferences.mock.calls.length).toBe(1);
        expect(api.putPreferences.mock.calls[0][1]).toBe('"1"');
        // After the response handler ran synchronously, lastKnownEtag
        // should be '"2"'. The next user edit should send "2".
        svc.update({ editorFontSize: 18 });
        TestBed.flushEffects();
        vi.advanceTimersByTime(600);
        expect(api.putPreferences.mock.calls.length).toBe(2);
        expect(api.putPreferences.mock.calls[1][1]).toBe('"2"');
      } finally {
        vi.useRealTimers();
      }
    });
    ...
    it('serializes in-flight writes (no second PUT until first completes)', async () => {
      ...
      try {
        ...
        api.putPreferences.mockReturnValue(firstPut.asObservable());
        auth.signInAs('u-1');
        TestBed.flushEffects();
        await svc.__waitForSync();
        svc.update({ theme: 'dark' });
    ...
    it('on 412 conflict refetches, replaces local, and emits a conflict event', async () => {
      ...
      try {
        const svc = TestBed.inject(PreferencesService);
        api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'system' }, '"1"')));
        auth.signInAs('u-1');
        TestBed.flushEffects();
        await svc.__waitForSync();
        ...
        api.putPreferences.mockReturnValue(
        ...
        api.getMe.mockReturnValue(of(makeUserResponse({ theme: 'light' }, '"5"')));
        svc.update({ theme: 'dark' });
        TestBed.flushEffects();
        vi.advanceTimersByTime(600);
        await Promise.resolve();
