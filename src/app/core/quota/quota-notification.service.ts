...
export class QuotaNotificationService {
  ...
  async notifyQuotaExceededManual(): Promise<void> {
    const ref = this.dialog.open<
      ...
      void,
      QuotaManualFullChoice
    >(QuotaManualFullDialogComponent, { autoFocus: 'dialog', width: '420px' });
    const choice = await firstValueFrom(ref.afterClosed());
  ...
  template: `
    ...
    <mat-dialog-content>
      <p i18n="@@quota.firstTime.body1">
        JotJSON free accounts keep up to 100 saved blobs. To make room, we automatically
        deleted your oldest saved blob and will keep doing that each time you hit the limit.
      </p>
    ...
    <mat-dialog-actions align="end">
      <button
        mat-button
        (click)="close('switch_to_manual')"
        i18n="@@quota.firstTime.manual"
...
export class QuotaFirstTimeDialogComponent {
  private readonly ref = inject<MatDialogRef<QuotaFirstTimeDialogComponent, QuotaFirstTimeChoice>>(
    MatDialogRef
  );
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-actions align="end">
      <button
        mat-button
        (click)="close('switch_to_auto')"
        i18n="@@quota.manualFull.switchAuto"
...
export class QuotaManualFullDialogComponent {
  private readonly ref = inject<
    MatDialogRef<QuotaManualFullDialogComponent, QuotaManualFullChoice>
  >(MatDialogRef);
