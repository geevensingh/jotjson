...
describe('RouteFocusService', () => {
  ...
  it('skips the first NavigationEnd', fakeAsync(() => {
    ...
    expect(main.focus)
      .withContext('first NavigationEnd is the cold-boot nav; service must not intervene')
  ...
  it('focuses <main id="main-content"> on subsequent NavigationEnd events', fakeAsync(() => {
    ...
    expect(main.focus)
      .withContext('second NavigationEnd should move focus to the new route landmark')
  ...
  it('adds tabindex="-1" to <main> if missing so focus() succeeds', fakeAsync(() => {
    ...
    expect(main.getAttribute('tabindex'))
      .withContext('non-interactive <main> needs tabindex=-1 to be focus()-able')
      .toBe('-1');
  }));
  ...
  it('preserves an existing tabindex on <main>', fakeAsync(() => {
    ...
    expect(main.getAttribute('tabindex'))
      .withContext('an existing tabindex must not be overwritten')
      .toBe('0');
  }));
  ...
  it('is a no-op when no <main id="main-content"> is in the DOM', fakeAsync(() => {
    ...
    expect(() => {
      ...
      .withContext('missing landmark must not throw')
      .not.toThrow();
  }));
});
