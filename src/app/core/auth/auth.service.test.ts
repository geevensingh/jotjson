...
describe('AuthService', () => {
  ...
  describe('telemetry events', () => {
    ...
    it('does not emit auth.signedIn for user-to-user transitions', () => {
      ...
      const auth = TestBed.inject(AuthService);
      callSetCurrentUser(auth, USER_A);
      loggerServiceSpy.event.mockClear();
    ...
    it('emits auth.signedOut before clearing the dev-auth user', async () => {
      ...
      vi.spyOn(currentUserSetter, 'setCurrentUser').mockImplementation((user) => {
    ...
    it('flushes auth.signedOut before redirecting for MSAL sign-out', async () => {
      ...
      telemetryServiceSpy.flush.mockImplementation(() => {
      ...
      vi.spyOn(fake, 'logoutRedirect').mockImplementation((request) => {
      ...
      expect(fake.logoutRedirectCalls).toBe(0);
      if (!resolveFlush) {
        expect.fail('Expected telemetry flush to start before logout redirect.');
        return;
      }
      resolveFlush();
  ...
  describe('dev-auth mode', () => {
    ...
    it('initializeFromRedirect does NOT call msal.initialize or handleRedirectPromise in dev mode', async () => {
      setDevAuth({ ...VALID_DEV_AUTH });
      const auth = freshAuth();
      const handleSpy = vi.spyOn(fake, 'handleRedirectPromise');
      await auth.initializeFromRedirect();
