...
describe('contrast utility (M6g-3)', () => {
  ...
  describe('meetsAA', () => {
    it('passes for black on white', () => {
      expect(meetsAA('#000000', '#ffffff')).toBeTrue();
    });

    it('fails for #777 on white (just under 4.5)', () => {
      expect(meetsAA('#777777', '#ffffff')).toBeFalse();
    });
    ...
    it('passes exactly at the AA threshold', () => {
      ...
      expect(meetsAA('#595959', '#ffffff')).toBeTrue();
    });
  ...
  describe('THEME_DEFAULTS', () => {
    ...
    it('the defaults themselves clear AA in their own theme', () => {
      expect(meetsAA(THEME_DEFAULTS.light.fg, THEME_DEFAULTS.light.bg)).toBeTrue();
      expect(meetsAA(THEME_DEFAULTS.dark.fg, THEME_DEFAULTS.dark.bg)).toBeTrue();
    });
  });
});
