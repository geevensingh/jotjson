...
describe('AppHeaderComponent (a11y)', () => {
  ...
  it('renders a skip-link as the first focusable element', () => {
    ...
    expect(
      skipLink,
      'app-header should expose a `.skip-link` anchor for keyboard users',
    ).not.toBeNull();
    expect(
      skipLink?.getAttribute('href'),
      'skip-link should target the main content landmark id',
    ).toBe('#main-content');
    ...
    expect(
      allFocusable[0],
      'skip-link must be the first focusable element so a single Tab reaches it',
  ...
  it('skip-link is visually hidden until it receives focus', () => {
    ...
    expect(
      idle.position,
      'skip-link uses position:absolute + clip-rect to stay visually hidden',
    ).toBe('absolute');
    expect(idle.clip, 'skip-link is clipped to a 0x0 rect when not focused').toBe(
    ...
    expect(
      focused.clip,
      'skip-link should drop the clip-rect when focused so its text is visible',
  ...
  it('wraps auth-side route links in a primary <nav> landmark', () => {
    ...
    expect(
      nav,
      'app-header should expose a <nav> landmark for the auth-side route links',
    ).not.toBeNull();
    expect(
      nav?.getAttribute('aria-label'),
      'the primary nav landmark needs an explicit accessible name',
    ).toBe('Primary');
  });
