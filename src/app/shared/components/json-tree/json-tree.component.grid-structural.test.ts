...
describe('JsonTreeComponent (.tree-row Grid template invariants -- v0.26.1)', () => {
  ...
  it('renders .tree-row with display: grid', async () => {
    ...
    expect(row, 'at least one real (non-probe) row must render').not.toBeNull();
    expect(
      getComputedStyle(row!).display,
      'the v0.26.1 Grid migration requires .tree-row to render as display: grid',
    ).toBe('grid');
  });
  ...
  it('grid-template-columns includes a 1fr filler track', async () => {
    ...
    expect(
      rightCluster,
      'every value/open row must render a .tree-row-right cluster',
    ).not.toBeNull();
  ...
  it('LongUnbreakableKey at 400px: deeply-nested long-key row does not overflow', async () => {
    ...
    expect(
      longKeyRow,
      'a row containing the AVeryLongUnbreakable... key must render after expandAll',
    ).toBeDefined();
  ...
  it('LongUnbreakableKey longKeyLongValue at 400px: key + value both renderable, row no overflow', async () => {
    ...
    expect(longLongRow, 'the long-key + long-value row must render after expandAll').toBeDefined();
    ...
    expect(
      key.getClientRects().length,
      'key must be in the render tree (display:none on element or any ancestor would fail this)',
    ).toBeGreaterThan(0);
    expect(
      getComputedStyle(key).visibility,
      'key must not be visibility:hidden (inherited from ancestor or set directly)',
    ).not.toBe('hidden');
    expect(
      valueEl.getClientRects().length,
      'value must be in the render tree (display:none on element or any ancestor would fail this)',
    ).toBeGreaterThan(0);
    expect(getComputedStyle(valueEl).visibility, 'value must not be visibility:hidden').not.toBe(
      'hidden',
    );
  ...
  it('.tree-row--close uses display: flex (not grid)', async () => {
    ...
    expect(
      closeRow,
      'an expanded fixture must surface at least one .tree-row--close row',
    ).not.toBeNull();
    expect(
      getComputedStyle(closeRow!).display,
  ...
  it('every direct child of .tree-row:not(.tree-row--close) has non-auto gridColumnStart or is position:absolute', async () => {
    ...
    expect(rows.length, 'at least one non-probe non-close row must render').toBeGreaterThan(0);
  ...
  it('probe row height equals first real row height within 2 px', async () => {
    ...
    expect(probe, 'probe row must render').not.toBeNull();
    expect(realRow, 'first real row must render').not.toBeNull();

    const probeHeight = probe!.getBoundingClientRect().height;
    const realHeight = realRow!.getBoundingClientRect().height;
    expect(
      Math.abs(probeHeight - realHeight),
  ...
  it('LongUnbreakableValue Parameters[0].Value at 400px: key + value present in render tree (structural CSS)', async () => {
    ...
    expect(row, 'overflowing leaf row must render').not.toBeNull();
    ...
    expect(getComputedStyle(key!).visibility, 'key must not be visibility:hidden').not.toBe(
      'hidden',
    );
    expect(
      valueEl!.getClientRects().length,
  ...
  it('long leading comment + long key at 400px: comment ellipsify contract holds + row no overflow', async () => {
    ...
    expect(longKeyRowInitial, 'a row with the long-key must render after expandAll').toBeDefined();
    const path = longKeyRowInitial!.getAttribute('data-path');
    expect(path, 'row must carry a data-path attribute').toBeTruthy();
    ...
    expect(longKeyRow, 'long-key row must still render').not.toBeNull();
    ...
    expect(leadingWrapper, 'leading wrapper must render').not.toBeNull();
    expect(leadingComment, 'leading comment must render').not.toBeNull();
    expect(key, 'key must render').not.toBeNull();
    ...
    expect(
      commentStyles.overflowX,
      '.tree-comment-leading must inherit overflow:hidden from .tree-comment',
    ).toBe('hidden');
    expect(
      commentStyles.textOverflow,
      '.tree-comment-leading must inherit text-overflow:ellipsis from .tree-comment',
    ).toBe('ellipsis');
    expect(
      commentStyles.whiteSpace,
      '.tree-comment-leading must inherit white-space:nowrap from .tree-comment',
    ).toBe('nowrap');
    expect(
      commentStyles.minWidth,
  ...
  it('CollapsedContainerSummary at 300px: every rendered row scrollWidth <= clientWidth + 1', async () => {
    ...
    expect(
      rows.length,
      'at least one non-probe row must render under default expansion',
    ).toBeGreaterThan(0);
    for (const row of rows) {
      expect(
        row.scrollWidth,
  ...
  it('value-side .tree-rule-icon--value is a descendant of .tree-row-value-cell, not a direct row child', async () => {
    ...
    expect(valueIcon, 'probe row must render its value-side rule icon stand-in').not.toBeNull();

    const valueCell = probe!.querySelector<HTMLElement>('.tree-row-value-cell');
    expect(valueCell, 'probe row must render the .tree-row-value-cell wrapper').not.toBeNull();
    expect(
      valueCell!.contains(valueIcon!),
  ...
  it('IsoDateAnnotations at 400px: date-annotation does not overflow into the right cluster', async () => {
    ...
    expect(dateRow, 'a date-annotated leaf row must render').not.toBeNull();
    ...
    expect(date, 'the date annotation must render when the pref is on').not.toBeNull();
  ...
  it('LongNumberValue: .tree-value-number declares the v0.26.1 ellipsify trio', async () => {
    ...
    expect(
      numberCell,
      'a .tree-value-number cell must render from the long-number fixture',
    ).not.toBeNull();
    ...
    expect(styles.textOverflow, '.tree-value-number must declare text-overflow:ellipsis').toBe(
      'ellipsis',
    );
    expect(
      styles.minWidth,
  ...
  it('changing treeFontSize preserves row layout invariants on both ends', async () => {
    ...
    expect(
      row!.scrollWidth,
      'at treeFontSize=13, row must not horizontally overflow',
    ).toBeLessThanOrEqual(row!.clientWidth + 1);
    ...
    expect(
      row!.scrollWidth,
      'at treeFontSize=20, row must not horizontally overflow either',
    ).toBeLessThanOrEqual(row!.clientWidth + 1);
  });
  ...
  it('EmptyArrayLeaf: empty-array leaf nests .tree-value-brace under .tree-value-container under .tree-row-value-cell', async () => {
    ...
    expect(tagsRow, 'empty-array leaf row $.tags must render').not.toBeNull();

    const valueCell = tagsRow!.querySelector<HTMLElement>('.tree-row-value-cell');
    expect(valueCell, 'value cell wrapper must render').not.toBeNull();
    ...
    expect(
      brace,
      'the .tree-value-brace ([]) must live inside .tree-value-container',
    ).not.toBeNull();
    expect(
      brace!.textContent?.trim(),
      'the brace span of an empty array must render the literal `[]`',
    ).toBe('[]');
  });
  ...
  it('issue #282: close row starts with .tree-row-leading > .tree-twisty.tree-spacer', async () => {
    ...
    const host = fixture.nativeElement as HTMLElement;
    const closeRow = host.querySelector<HTMLElement>('.tree-row.tree-row--close');
    expect(closeRow, 'at least one .tree-row--close must render after expandAll').not.toBeNull();

    const firstChild = closeRow!.firstElementChild as HTMLElement | null;
    expect(firstChild, 'close row must have at least one child element').not.toBeNull();
    expect(
      firstChild!.classList.contains('tree-row-leading'),
    ...
    expect(
      leadingFirstChild,
      '.tree-row-leading on a close row must contain a child element',
    ).not.toBeNull();
    expect(
      leadingFirstChild!.classList.contains('tree-twisty'),
      `.tree-row-leading's first child on a close row must be .tree-twisty (the spacer); ` +
        `got class="${leadingFirstChild!.className}".`,
    ).toBe(true);
    expect(
      leadingFirstChild!.classList.contains('tree-spacer'),
  ...
  it('issue #282: close-row brace left edge equals sibling-key left edge at same indent level', async () => {
    ...
    const host = fixture.nativeElement as HTMLElement;
    const closeRow = host.querySelector<HTMLElement>('.tree-row.tree-row--close');
    expect(closeRow, 'close row for `arr` must render').not.toBeNull();
    const brace = closeRow!.querySelector<HTMLElement>('.tree-value-brace');
    expect(brace, 'close-row brace must render').not.toBeNull();

    const siblingLeaf = host.querySelector<HTMLElement>('[data-path="$.next"]');
    expect(
      siblingLeaf,
      'level-1 sibling leaf row `next` must render at same indent as `arr` close',
    ).not.toBeNull();
    const siblingKey = siblingLeaf!.querySelector<HTMLElement>('.tree-key');
    expect(siblingKey, 'sibling leaf must have a .tree-key element').not.toBeNull();
  ...
  it('issue #282: close-row height equals probe row height within 2 px', async () => {
    ...
    const closeRow = host.querySelector<HTMLElement>('.tree-row.tree-row--close');
    expect(probe, 'probe row must render').not.toBeNull();
    expect(closeRow, 'a close row must render after expandAll').not.toBeNull();

    const probeHeight = probe!.getBoundingClientRect().height;
    const closeHeight = closeRow!.getBoundingClientRect().height;
    expect(
      Math.abs(probeHeight - closeHeight),
  ...
  it('issue #282: long leading-close comment at level 4 + 360px viewport: no overflow, ellipsifies', async () => {
    ...
    expect(
      closeRow,
      'the level-4 close row for `items` must render with the leading-close comment',
    ).not.toBeNull();
    ...
    expect(
      leadingComment,
      'the leading-close comment must render inside the close row',
    ).not.toBeNull();
    ...
    expect(leadingWrapper, 'close row must carry a .tree-row-leading').not.toBeNull();
    expect(
      leadingWrapper!.contains(leadingComment!),
    ...
    expect(
      commentStyles.overflowX,
      '.tree-comment-leading--close must inherit overflow:hidden from .tree-comment',
    ).toBe('hidden');
    expect(
      commentStyles.textOverflow,
      '.tree-comment-leading--close must inherit text-overflow:ellipsis',
    ).toBe('ellipsis');
    expect(
      commentStyles.whiteSpace,
      '.tree-comment-leading--close must inherit white-space:nowrap',
    ).toBe('nowrap');
    expect(
      commentStyles.minWidth,
