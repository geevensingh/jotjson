...
describe('DecodedValueDialogComponent', () => {
  ...
  let loggerEvent: Mock;
  let liveAnnounce: Mock;
  ...
  function createWith(data: DecodedValueDialogData) {
    ...
    loggerEvent = vi.fn();
    liveAnnounce = vi.fn().mockResolvedValue(undefined);
  ...
  describe('copy', () => {
    ...
    it('does not throw when the underlying copy resolves to false (failed path)', () => {
      copyWithToast = vi.fn().mockResolvedValue(false);
      TestBed.resetTestingModule();
      TestBed.configureTestingModule({
        imports: [DecodedValueDialogComponent],
        providers: [
          ...
          { provide: ClipboardCopyService, useValue: { copyWithToast } },
          { provide: LoggerService, useValue: { event: vi.fn() } },
          {
            provide: LiveAnnouncer,
            useValue: { announce: vi.fn().mockResolvedValue(undefined) },
          },
        ],
      });
      const fixture = TestBed.createComponent(DecodedValueDialogComponent);
    ...
    it('does not throw when the underlying copy rejects (unsupported environment)', () => {
      copyWithToast = vi.fn().mockRejectedValue(new Error('unsupported'));
      TestBed.resetTestingModule();
      TestBed.configureTestingModule({
        imports: [DecodedValueDialogComponent],
        providers: [
  ...
  describe('mangling decode toggle', () => {
    ...
    it('renders the toggle in the off state when the heuristic fires', () => {
      ...
      expect(fixture.componentInstance.decoded()).toBe(false);
    ...
    it('Copy button mirrors the toggle: raw when off, decoded when on', () => {
      ...
      rawCopy!.click();
      expect(copyWithToast).toHaveBeenCalledTimes(1);
      expect(copyWithToast.mock.lastCall![0]).toBe(MANGLED_RESPONSE);
      ...
      decodedCopy!.click();
      expect(copyWithToast.mock.calls.length).toBe(2);
      const written = copyWithToast.mock.lastCall![0] as string;
    ...
    it('shows the Apply button only in decoded mode, closes with applyDecoded, announces and logs', () => {
      ...
      expect(liveAnnounce.mock.lastCall![0]).toBe('Applying decoded value to source.');
      const applyCalls = loggerEvent.mock.calls.filter((args) => args[0] === 'tree.decoded.apply');
    ...
    it('announces the new state via LiveAnnouncer on every flip', () => {
      ...
      expect(liveAnnounce.mock.calls[0]![0]).toBe('Showing "??" markers as line breaks.');
      expect(liveAnnounce.mock.calls[1]![0]).toBe('Showing raw value.');
    });
    ...
    it('emits tree.decoded.manglingToggle with the post-flip "to" prop on every flip', () => {
      ...
      cmp.toggleDecoded(true);
      cmp.toggleDecoded(false);
      const calls = loggerEvent.mock.calls.filter(
        (args) => args[0] === 'tree.decoded.manglingToggle',
      );
