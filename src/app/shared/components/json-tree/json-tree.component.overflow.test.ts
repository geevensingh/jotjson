...
describe('JsonTreeComponent (row-width overflow)', () => {
  ...
  it('constrains the cdk-virtual-scroll-content-wrapper to the viewport width when a row value is much wider', async () => {
    ...
    expect(
      viewport,
      'the cdk-virtual-scroll-viewport with class .tree-viewport must render',
    ).toBeTruthy();
    ...
    expect(
      valueCells.length,
      'expandAll should surface at least one string-value cell from the fixture',
    ).toBeGreaterThan(0);
  ...
  it('v0.23.1: keeps the twisty spacer at its natural width when the row has a much-wider value sibling', async () => {
    ...
    expect(twisty, 'leaf row must render a .tree-twisty spacer').not.toBeNull();
  ...
  it('v0.23.1: aligns the .tree-key X-position against padding-left + twisty width on an overflowing row', async () => {
    ...
    expect(key, 'leaf row must render a .tree-key').not.toBeNull();
    ...
    expect(
      Math.abs(actualKeyOffset - expectedKeyOffset),
  ...
  it('v0.23.1: wires matTooltipClass="jj-tooltip-wide" onto the leaf value-string tooltip', async () => {
    ...
    expect(
      debugEl,
      'a .tree-value-string MatTooltip directive must be present after expandAll',
    ).toBeDefined();
