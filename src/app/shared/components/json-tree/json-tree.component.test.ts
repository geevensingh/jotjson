...
describe('JsonTreeComponent', () => {
  ...
  async function createWith(
    ...
    loggerOverride?: Mocked<LoggerService>,
  ): Promise<void> {
  ...
  async function createWithLoggerSpy(value: unknown): Promise<Mocked<LoggerService>> {
    const logger = {
      event: vi.fn(),
      info: vi.fn(),
      warn: vi.fn(),
  ...
  it('scales the type badge font-size with treeFontSize', async () => {
    ...
    try {
      ...
      expect(badge, 'expected a .tree-type-badge to be rendered').toBeTruthy();
  ...
  it('overrides Material tree-node font-size so large treeFontSize values actually render', async () => {
    ...
    try {
      ...
      expect(node, 'expected a tree row to be rendered').toBeTruthy();
  ...
  describe('row density (em-based icon chrome)', () => {
    ...
    it('inherits tree font-size into the kebab pill via font: inherit', async () => {
      ...
      try {
        ...
        expect(pill, 'expected a .tree-kebab-pill to be rendered').toBeTruthy();
    ...
    it('scales the twisty button (1.1em) with tree font size', async () => {
      ...
      try {
        ...
        expect(twisty, 'expected an active .tree-twisty (non-spacer) to be rendered').toBeTruthy();
    ...
    it('keeps the probe row content height at or below 18px at the smallest font', async () => {
      ...
      try {
        ...
        expect(probe, 'expected a .tree-row--probe to be rendered').toBeTruthy();
  ...
  describe('root() and path formatting', () => {
    ...
    it('matches the shared manual-highlight path corpus', async () => {
      for (const fixtureEntry of HIGHLIGHT_PATH_FIXTURES) {
        ...
        expect(paths, `formatPath should emit ${fixtureEntry.path}`).toContain(fixtureEntry.path);
      }
    });
  ...
  describe('tree build slow telemetry', () => {
    ...
    it('marks only the first tree.build.slow emission as cold', async () => {
      ...
      fixture.detectChanges();
      const buildCalls = eventSpy.mock.calls.filter((args) => args[0] === 'tree.build.slow');
  ...
  describe('tree render slow telemetry', () => {
    ...
    it('drops a stale tree.render.slow measurement when value changes again', async () => {
      ...
      const renderCalls = eventSpy.mock.calls.filter((args) => args[0] === 'tree.render.slow');
  ...
  describe('initial expansion auto-fit', () => {
    ...
    it('with auto-fit ON, picks K via the algorithm and calls expandToLevel(K, true)', async () => {
      ...
      const expandSpy = vi.spyOn(cmp, 'expandToLevel');
  ...
  describe('searchHits', () => {
    ...
    it('matches string values', () => {
      ...
      expect(hits.has('$.alpha')).toBe(true);
      expect(hits.has('$.beta.gamma')).toBe(true);
    });
    ...
    it('is case-sensitive when preference is set', () => {
      ...
      const hits = cmp.searchHits();
      expect(hits.has('$.beta.gamma')).toBe(true);
      expect(hits.has('$.alpha')).toBe(false);
    });
    ...
    it('pins the contains-vs-exact haystack asymmetry for string values', async () => {
      ...
      cmp.search.set('"hello"');
      expect(cmp.searchHits().has('$.a')).toBe(true);
      cmp.search.set('hello');
      expect(cmp.searchHits().has('$.a')).toBe(true);
      ...
      expect(cmp.searchHits().has('$.a')).toBe(true);
      cmp.search.set('"hello"');
      expect(cmp.searchHits().has('$.a')).toBe(false);
    });
    ...
    it('matches regex in regex mode', () => {
      ...
      expect(hits.has('$.alpha')).toBe(true);
      expect(hits.has('$.beta.gamma')).toBe(true);
      expect(hits.has('$.beta.delta')).toBe(false);
    });
    ...
    it('skips the tree walk on invalid regex (regression: matcher returns null)', () => {
      ...
      const haystackSpy = vi.spyOn(target, 'valueHaystack');
  ...
  describe('searchHits regex mode value haystack', () => {
    ...
    it('regex \\n metachar matches a real newline in the raw value', async () => {
      ...
      const hits = cmp.searchHits();
      expect(hits.has('$.note')).toBe(true);
    });
    ...
    it('regex ^$ matches an empty string value', async () => {
      ...
      cmp.search.set('^$');
      const hits = cmp.searchHits();
      expect(hits.has('$.blank')).toBe(true);
    });
  });
  ...
  describe('search by value type', () => {
    ...
    it('empty query + active type lists every node of that type (navigator mode)', () => {
      ...
      cmp.search.set('');
      const hits = cmp.searchHits();
      expect(hits.has('$.count')).toBe(true);
      // Non-integer leaves are excluded.
      expect(hits.has('$.ratio')).toBe(false);
      expect(hits.has('$.id')).toBe(false);
      expect(hits.has('$.active')).toBe(false);
      // The root sentinel is never a hit.
      expect(hits.has('$')).toBe(false);
    });
    ...
    it('empty query + uuid filter finds the uuid leaf only', () => {
      ...
      cmp.search.set('');
      const hits = cmp.searchHits();
      expect(hits.has('$.id')).toBe(true);
      expect(hits.has('$.email')).toBe(false);
    });
    ...
    it('scope=keys + empty query + type filter still lists all candidates (text scope is irrelevant)', () => {
      ...
      cmp.search.set('');
      expect(cmp.searchHits().has('$.active')).toBe(true);
    });

    it('searchActive is true when only the type filter is set', () => {
      cmp.search.set('');
      expect(cmp.searchActive()).toBe(false);
  ...
  describe('expandAll / expandToLevel / collapseAll', () => {
    ...
    it('expandAll expands every container node', () => {
      ...
      const walk = (n: typeof root): void => {
        ...
        expect(cmp.__getHelpersForTesting().isExpanded(n), n.pathString).toBe(true);
    ...
    it('collapseAll collapses every node', () => {
      ...
      cmp.collapseAll();
      const root = cmp.root()!;
      expect(cmp.__getHelpersForTesting().isExpanded(root)).toBe(false);
    });
    ...
    it('expandToLevel(n) expands only nodes with depth < n', () => {
      ...
      cmp.expandToLevel(2);
      const root = cmp.root()!;
      expect(cmp.__getHelpersForTesting().isExpanded(root)).toBe(true); // depth 0
      const a = root.children!.find((c) => c.segment === 'a')!;
      expect(cmp.__getHelpersForTesting().isExpanded(a)).toBe(true); // depth 1
      const b = a.children!.find((c) => c.segment === 'b')!;
      expect(cmp.__getHelpersForTesting().isExpanded(b)).toBe(false); // depth 2 should NOT be expanded
    });
  });
  ...
  describe('key display (control chars in keys)', () => {
    ...
    it('routes numeric segments to .tree-index, not .tree-key', async () => {
      ...
      expect(indexTexts).toEqual(expect.arrayContaining(['0:', '1:']));
    });
  });
  ...
  describe('JSONC comment rendering (M7k)', () => {
    ...
    it('renders a trailing comment on a leaf row as a sibling of tree-row-right', async () => {
      ...
      expect(leafRow, 'leaf row should be rendered').not.toBeNull();
      ...
      expect(trailing, 'trailing slot must render').not.toBeNull();
      ...
      expect(
        trailingWrapper,
        '.tree-row-trailing wrapper is a direct child of the leaf row',
      ).not.toBeNull();
      expect(
        trailingWrapper!.contains(trailing!),
        'trailing comment is inside the trailing wrapper',
      ).toBe(true);
      ...
      expect(
        right.querySelector('.tree-comment-trailing'),
        'trailing slot must NOT be inside tree-row-right',
      ).toBeNull();
    });
    ...
    it('renders the trailing comment AFTER the value and BEFORE tree-row-right on leaf rows', async () => {
      ...
      expect(valueEl, 'value span').not.toBeNull();
      expect(trailingEl, 'trailing comment').not.toBeNull();
      expect(rightEl, 'tree-row-right').not.toBeNull();
      ...
      expect(
        valueBeforeTrailing,
        'trailing must follow the value in document order',
      ).toBeGreaterThan(0);
      expect(
        trailingBeforeRight,
        'tree-row-right must follow the trailing comment',
      ).toBeGreaterThan(0);
    });
    ...
    it('renders the trailing comment AFTER the date annotation on string rows', async () => {
      ...
      const trailing = leafRow.querySelector('.tree-comment-trailing');
      expect(dateAnn, 'date annotation should render').not.toBeNull();
      expect(trailing, 'trailing comment should render').not.toBeNull();
      const positionMask = dateAnn!.compareDocumentPosition(trailing!);
      // DOCUMENT_POSITION_FOLLOWING = 4
      expect(
        positionMask & Node.DOCUMENT_POSITION_FOLLOWING,
        'trailing must follow date annotation in document order',
      ).toBeGreaterThan(0);
    });
    ...
    it('renders the trailing comment BEFORE the kebab on leaf rows', async () => {
      ...
      expect(trailing, 'trailing comment').not.toBeNull();
      expect(kebab, 'kebab').not.toBeNull();
      const positionMask = trailing!.compareDocumentPosition(kebab!);
      expect(
        positionMask & Node.DOCUMENT_POSITION_FOLLOWING,
        'kebab must follow trailing comment in document order',
      ).toBeGreaterThan(0);
    });
    ...
    it('does not impose a max-width on .tree-comment (flex-driven shrink instead)', async () => {
      ...
      try {
        ...
        expect(trailing, 'trailing slot should render').toBeTruthy();
    ...
    it('renders the trailing-on-close comment on the container close row', async () => {
      ...
      expect(closeRow, 'close row should be rendered').not.toBeNull();
      const trailing = closeRow!.querySelector('.tree-comment-trailing');
      expect(trailing, 'trailing slot on close row').not.toBeNull();
    ...
    it('renders both closeLeading (before brace) and closeTrailing (after brace) on the container close row', async () => {
      ...
      expect(leading, 'closeLeading slot should render').not.toBeNull();
      expect(leading!.textContent?.trim()).toBe('end of section for bar');

      expect(trailing, 'closeTrailing slot should render').not.toBeNull();
      expect(trailing!.textContent?.trim()).toBe('closing comment of foo');

      expect(brace, 'close brace should render').not.toBeNull();

      const leadingBeforeBrace =
        leading!.compareDocumentPosition(brace!) & Node.DOCUMENT_POSITION_FOLLOWING;
      expect(leadingBeforeBrace, 'closeLeading must appear before the close brace').toBeGreaterThan(
        0,
      );

      const trailingAfterBrace =
        brace!.compareDocumentPosition(trailing!) & Node.DOCUMENT_POSITION_FOLLOWING;
      expect(trailingAfterBrace, 'closeTrailing must appear after the close brace').toBeGreaterThan(
        0,
      );
    });
    ...
    it('merges closeLeading and closeTrailing into the trailing slot on an empty inline container', async () => {
      ...
      expect(trailing, 'merged trailing slot should render on empty container').toBeDefined();
      ...
      expect(debugEl, 'trailing-comment tooltip directive').toBeDefined();
    ...
    it('renders an open-row trailing comment on a container open row', async () => {
      ...
      expect(openRow, 'foo open row should render').not.toBeNull();
      ...
      expect(openRowTrailing, 'open-row trailing slot must render').not.toBeNull();
      expect(openRowTrailing!.textContent?.trim()).toBe('explaination of foo');
      expect(closeRowTrailing, 'close-row trailing slot must NOT render').toBeFalsy();
      ...
      expect(
        openRowTrailingWrapper,
        '.tree-row-trailing wrapper must render as a direct grid-item',
      ).not.toBeNull();
      expect(
        openRowTrailingWrapper!.contains(openRowTrailing!),
        'trailing comment must live inside the trailing wrapper',
      ).toBe(true);
      ...
      expect(
        positionMask & Node.DOCUMENT_POSITION_FOLLOWING,
        'tree-row-right must follow trailing comment',
      ).toBeGreaterThan(0);
    });
    ...
    it('renders a (+1) count badge when leading has 2 stacked bodies', async () => {
      ...
      expect(badge, 'count badge should render').toBeDefined();
    ...
    it('renders a count badge on closeLeading when it has 2 stacked bodies', async () => {
      ...
      expect(closeRow, 'close row should render').not.toBeNull();
      const [badge] = visibleElements('.tree-comment-count', closeRow!);
      expect(badge, 'closeLeading badge should render').toBeDefined();
      expect(badge!.textContent).toMatch(/\b1\b/);
    });
    ...
    it('renders a count badge on a primitive trailing slot when it has 2 stacked bodies', async () => {
      ...
      expect(leafRow, 'leaf row should render').not.toBeNull();
      const [badge] = visibleElements('.tree-comment-count', leafRow!);
      expect(badge, 'trailing badge should render').toBeDefined();
      expect(badge!.textContent).toMatch(/\b1\b/);
    });
    ...
    it('renders a (+2) badge when empty-container trailing merge has 3 total bodies', async () => {
      ...
      expect(trailing, 'merged trailing slot should render').toBeDefined();
    ...
    it('paints the count badge in the same color as the body comment (computed-styles regression net)', async () => {
      ...
      try {
        ...
        const [badge] = visibleElements('.tree-comment-count');
        expect(body, 'body comment should render').toBeDefined();
        expect(badge, 'count badge should render').toBeDefined();
    ...
    it('does not compound margins between slot wrapper and inner comment (hoisting regression net)', async () => {
      ...
      try {
        ...
        expect(inner, 'leading comment should render').toBeDefined();
    ...
    it('keeps the count cluster intact when a leading comment is much longer than the row', async () => {
      ...
      try {
        ...
        expect(userRow, 'user open row should render').not.toBeNull();
        ...
        expect(rowRight, 'row-right cluster').not.toBeNull();
        expect(count, 'count span').not.toBeNull();
  ...
  describe('selection highlighting', () => {
    ...
    function findRow(path: string): HTMLElement {
      ...
      if (selected) {
        expect(
          ...
          `aria-selected on selected row for ${path}`,
        ).toBe('true');
      }
    ...
    it('highlights the ancestor chain up to root for a deep selection', async () => {
      ...
      expect(ancestors.has('$.a.b.c')).toBe(false);
    });
    ...
    it('highlights matching primitive values type-aware (1 != "1")', async () => {
      ...
      expect(matches.has('$.c')).toBe(true);
      expect(matches.has('$.b')).toBe(false);
      expect(matches.has('$.d')).toBe(false);
      expect(matches.has('$.a')).toBe(false); // selected itself excluded
    });
    ...
    it('renders a match badge on matching rows but not on the selected row', async () => {
      ...
      expect(matchRow.classList.contains('is-match')).toBe(true);
      expect(matchRow.classList.contains('is-selected')).toBe(false);
      // Badge lives inside the trailing .tree-row-right strip, not as a
      // leading element that would shift the row's left content.
      expect(badges[0].parentElement?.classList.contains('tree-row-right')).toBe(true);
    });
    ...
    it('selects correctly through keys with special characters', async () => {
      ...
      expect(cmp.ancestorPaths().has('$')).toBe(true);
    });
    ...
    it('does not select when clicking the twisty toggle', async () => {
      ...
      expect(twisty, 'expected a twisty toggle button').toBeTruthy();
      twisty.click();
      fixture.detectChanges();
      expect(cmp.selectedPath()).toBeNull();
    });
    ...
    it('applies all four highlight classes simultaneously (priority is a CSS concern)', async () => {
      ...
      expect(xRow.classList.contains('is-selected')).toBe(true);
      expect(xRow.classList.contains('is-search-hit')).toBe(true);
      // a11y carry-over from PR #229: aria-selected must still attach to
      // the selected row after Phase 2's template rewrite.
  ...
  describe('manual highlights', () => {
    ...
    it('renders manual highlight classes and colors for direct and cascaded rows', async () => {
      ...
      expect(parentRow.classList.contains('has-manual-highlight')).toBe(true);
      expect(parentRow.style.getPropertyValue('--manual-highlight').trim()).toBe('#ffe082');
      expect(childRow.classList.contains('has-manual-highlight')).toBe(true);
      expect(childRow.style.getPropertyValue('--manual-highlight').trim()).toBe('#ffe082');
      expect(directRow.classList.contains('has-manual-highlight')).toBe(true);
      expect(directRow.style.getPropertyValue('--manual-highlight').trim()).toBe('#abcdef');
      expect(siblingRow.classList.contains('has-manual-highlight')).toBe(false);
    ...
    async function openMenuFor(path: string): Promise<void> {
      ...
      expect(kebab, `found a kebab for ${path}`).toBeTruthy();
      kebab!.click();
      fixture.detectChanges();
      await Promise.resolve();
      fixture.detectChanges();
    ...
    async function openHighlightFlyout(
      ...
      expect(flyout, `opened flyout for ${label} (${scope})`).toBeTruthy();
    ...
    describe('context menu highlight visibility', () => {
      it('shows Highlight and Highlight tree for container rows', async () => {
        ...
        expect(cmp.showHighlight(node)).toBe(true);
        expect(cmp.showHighlightTree(node)).toBe(true);
        expect(cmp.showRemoveHighlight(node)).toBe(false);
        expect(cmp.showRemoveTreeHighlight(node)).toBe(false);
      });
      ...
      it('shows only Highlight for primitive rows without inherited cascade', async () => {
        ...
        expect(cmp.showHighlight(node)).toBe(true);
        expect(cmp.showHighlightTree(node)).toBe(false);
        expect(cmp.showRemoveHighlight(node)).toBe(false);
        expect(cmp.showRemoveTreeHighlight(node)).toBe(false);
      });
      ...
      it('shows only tree-scope highlight actions for closing-brace rows', async () => {
        ...
        expect(cmp.showHighlight(node)).toBe(false);
        expect(cmp.showHighlightTree(node)).toBe(true);
        expect(cmp.showRemoveHighlight(node)).toBe(false);
        expect(cmp.showRemoveTreeHighlight(node)).toBe(true);
      });
      ...
      it('shows Remove highlight only for an own non-cascade entry', async () => {
        ...
        expect(cmp.showRemoveHighlight(node)).toBe(true);
        expect(cmp.showRemoveTreeHighlight(node)).toBe(false);
      });
    ...
    describe('context menu close behavior', () => {
      ...
      it('closes the menu chain after clicking a single-row swatch', async () => {
        ...
        expect(isAnyMenuPanelOpen(), 'all menu panels closed').toBe(false);
      });
      ...
      it('closes the menu chain on idempotent re-apply (same color, same cascade)', async () => {
        ...
        expect(events, 'no emit on idempotent re-apply').toEqual([]);
        expect(isAnyMenuPanelOpen(), 'menu still closes for selection feedback').toBe(false);
      });
      ...
      it('clicking the Highlight item directly applies the preferred color and closes', async () => {
        ...
        expect(highlightFlyoutCount(), 'flyout did not open').toBe(0);
        expect(isAnyMenuPanelOpen(), 'all menu panels closed').toBe(false);
      });
      ...
      it('keyboard Enter on the Highlight item applies preferred, closes, restores row focus, and reports inputMode=keyboard', async () => {
        ...
        expect(
          cmp.focusedPath(),
          'focusedPath returns to the originating row on keyboard apply',
        ).toBe('$.leaf');
      });
      ...
      it('mouse click on the Highlight item reports inputMode=mouse and does not yank focus to the row', async () => {
        ...
        expect(cmp.focusedPath(), 'mouse path does not yank focus to the row').toBe('$');
      });
    });
    ...
    describe('context menu keyboard navigation (issue #100)', () => {
      ...
      it('focuses the Preferred bar when the swatch menu opens (bridge via onSwatchMenuOpened)', async () => {
        ...
        const preferredBar = flyout.querySelector<HTMLButtonElement>('.preferred-bar');
        expect(preferredBar, 'preferred bar present').toBeTruthy();
        expect(document.activeElement, 'keyboard-bridge moves focus into the flyout').toBe(
          preferredBar,
        );
      });
      ...
      it('returns focus to the originating row after a keyboard apply', async () => {
        ...
        expect(
          cmp.focusedPath(),
          'focusedPath snaps back to the originating row on keyboard apply',
        ).toBe('$.leaf');
        // Verify isAnyMenuPanelOpenLocal helper is exercised so the
        // close-state assertion is part of the keyboard contract too.
        expect(isAnyMenuPanelOpenLocal(), 'apply path also closes the menu chain').toBe(false);
      });
      ...
      it('clicking the flyout padding (root host) does not emit apply or close the menu', async () => {
        ...
        expect(events, 'no highlight emitted from padding click').toEqual([]);
        expect(
          highlightFlyoutCountLocal(),
          'flyout stays open when the padding strip is clicked',
        ).toBeGreaterThan(0);
      });
      ...
      it('does not preventDefault on Escape inside the flyout (delegates to mat-menu)', async () => {
        ...
        expect(escape.defaultPrevented, 'Escape passes through to mat-menu').toBe(false);
      });
    });
  });
  ...
  describe('date annotations', () => {
    ...
    it('relative-time portion refreshes when the now signal ticks', async () => {
      ...
      try {
        ...
        vi.setSystemTime(new Date(baseNow + 65_000));
  ...
  describe('selection sync API (issue #42)', () => {
    it('hasPath returns true for known paths and false for unknown', async () => {
      ...
      expect(cmp.hasPath('$')).toBe(true);
      expect(cmp.hasPath('$.a')).toBe(true);
      expect(cmp.hasPath('$.b')).toBe(true);
      expect(cmp.hasPath('$.b.c')).toBe(true);
      expect(cmp.hasPath('$.nonexistent')).toBe(false);
      expect(cmp.hasPath('not-a-path')).toBe(false);
    });
    ...
    it('selectByPathString expands ancestors so the row is visible', async () => {
      ...
      expect(cmp.__getHelpersForTesting().isExpanded(aNode)).toBe(false);
      expect(cmp.__getHelpersForTesting().isExpanded(bNode)).toBe(false);
      cmp.selectByPathString('$.a.b.c');
      expect(cmp.__getHelpersForTesting().isExpanded(aNode)).toBe(true);
      expect(cmp.__getHelpersForTesting().isExpanded(bNode)).toBe(true);
    });
    ...
    it('expandNodeAtPath expands exactly the named node (no descendants, no ancestors)', async () => {
      ...
      expect(cmp.__getHelpersForTesting().isExpanded(targetNode), 'target expanded').toBe(true);
      expect(
        cmp.__getHelpersForTesting().isExpanded(outerNode),
        'outer (ancestor) NOT expanded',
      ).toBe(false);
      expect(
        cmp.__getHelpersForTesting().isExpanded(innerNode),
        'inner (descendant) NOT expanded',
      ).toBe(false);
    });
    ...
    it('expandNodeAtPath persists across re-parse via pathString trackBy', async () => {
      ...
      expect(
        cmp.__getHelpersForTesting().isExpanded(newANode),
        'post-mutation node honors pre-mutation expand call',
      ).toBe(true);
    });
    ...
    it('selectionChange emits structural path when selectedPath changes (e.g. via user click)', async () => {
      ...
      fixture.detectChanges();
      expect(
        events.some((path) => path !== null && path.length === 1 && path[0] === 'a'),
        'expected a selectionChange event with path ["a"]',
      ).toBe(true);
    });
    ...
    it('selectionChange emits on programmatic selectByPathString', async () => {
      ...
      expect(matched, 'expected path ["a","b"]').toBeTruthy();
    });
    ...
    it('does not re-emit selectionChange when only the value re-flows (same selectedPath, new nodeIndex)', async () => {
      ...
      expect(events, 'no spurious re-emit on value re-flow').toEqual([]);
      ...
      fixture.componentRef.setInput('value', { a: 1, b: 2 });
      fixture.detectChanges();
      expect(events, 'no spurious re-emit at deeper path').toEqual([]);
    });
    ...
    it('still emits when selectedPath references a path the previous nodeIndex did not have', async () => {
      ...
      expect(newKeyEvent, 'expected an event for ["newKey"]').toBeTruthy();
    });
  });
  ...
  describe('selectByPathString defer/retry (issue #266)', () => {
    ...
    it('rapid defers overwrite pending; only the final value emits when nodeIndex settles', async () => {
      ...
      expect(stale, 'no stale intermediate emissions').toEqual([]);
  ...
  describe('embedded JSON extraction UI', () => {
    ...
    async function openMenuFor(pathString: string): Promise<void> {
      ...
      expect(kebab, `found a kebab for ${pathString}`).toBeTruthy();
      kebab!.click();
      fixture.detectChanges();
      await Promise.resolve();
      fixture.detectChanges();
    ...
    describe('opening the dialog', () => {
      ...
      it('kebab "Open decoded value" entry opens the same dialog', async () => {
        ...
        try {
          ...
          expect(item, 'decoded menu item should be present').toBeTruthy();
          item!.click();
          fixture.detectChanges();
          expect(open).toHaveBeenCalledTimes(1);
          const args = open.mock.lastCall!;
  ...
  describe('row context menu (M7q)', () => {
    ...
    describe('onRowContextMenu', () => {
      it('sets contextNode and captures cursor coords without mutating selection', async () => {
        ...
        expect(cmp.selectedPath(), 'right-click must not change selection').toBe('$.beta');
      ...
      it('does not mutate selectedPath when no prior selection exists', async () => {
        ...
        fixture.detectChanges();
        const node = nodeAt('$.alpha');
        expect(cmp.selectedPath(), 'precondition: nothing selected').toBeNull();
        cmp.onRowContextMenu(ctxEvent(), node);
        expect(cmp.selectedPath(), 'right-click must leave selection null').toBeNull();
        expect(cmp.contextNode()?.pathString).toBe('$.alpha');
      });
      ...
      it('ignores contextmenu fired on an interactive descendant (e.g. the kebab pill)', async () => {
        ...
        expect(pill, 'expected an interactive data-row kebab button').toBeTruthy();
        const ev = new MouseEvent('contextmenu', {
          clientX: 100,
          clientY: 100,
          bubbles: true,
    ...
    describe('onKebabClick', () => {
      ...
      it("logs tree.contextMenu.opened with source: 'kebab'", async () => {
        ...
        const node = nodeAt('$.alpha');
        const ev = new MouseEvent('click', { bubbles: true, cancelable: true });
        const info = vi.spyOn(TestBed.inject(LoggerService), 'info');
        cmp.onKebabClick(ev, node);
    ...
    describe('window.blur dismissal (M7q + JJ_MENU_IMPORTS)', () => {
      it('closes the open row context menu when window.blur fires', async () => {
        ...
        expect(row, 'found a row to right-click').toBeTruthy();
        ...
        try {
          ...
          expect(cmp.ctxTrigger()?.menuOpen).toBe(true);
          ...
          expect(cmp.ctxTrigger()?.menuOpen).toBe(false);
        } finally {
    ...
    describe('onBreadcrumbContextMenu', () => {
      ...
      it('opens the menu and captures cursor coords without mutating selection', async () => {
        ...
        expect(
          cmp.selectedPath(),
          'right-click on a breadcrumb chip must not change selection',
        ).toBe('$.foo.bar');
        expect(cmp.ctxX()).toBe(100);
        expect(cmp.ctxY()).toBe(200);
      });
      ...
      it('does not fire selectionChange', async () => {
        ...
        fixture.detectChanges();
        expect(events, 'right-click must not emit a selectionChange event').toEqual([]);
      });
    ...
    describe('collapseFromHere', () => {
      it("collapses the clicked container without clearing descendants' state", async () => {
        ...
        expect(cmp.__getHelpersForTesting().isExpanded(outer), 'clicked row collapses').toBe(false);
        ...
        expect(
          cmp.__getHelpersForTesting().isExpanded(mid),
          "descendant's expansion state preserved",
        ).toBe(true);
      });
    });
    ...
    describe('isolate / collapseSiblings', () => {
      it('truth-table (non-empty, empty): single Isolate shown; collapses peers; emits tree.contextMenu.isolate', async () => {
        ...
        const infoSpy = vi.spyOn(TestBed.inject(LoggerService), 'info');
      ...
      it('telemetry: each ID is emitted with no payload (no user content)', async () => {
        ...
        for (const args of infoSpy.mock.calls) {
    ...
    describe('visibility predicates', () => {
      ...
      it('showExpandToDepth: hides every +N when subtree is already fully expanded (Bug 2)', async () => {
        ...
        for (let depth = 1; depth <= 5; depth++) {
          expect(
            cmp.showExpandToDepth(outer, depth),
            `+${depth} should hide when fully expanded`,
          ).toBe(false);
        }
      });
    ...
    describe('onRowDblClick', () => {
      ...
      it('expands a collapsed container on dblclick', async () => {
        ...
        const writeText = vi.fn().mockResolvedValue(undefined);
        const node = nodeAt('$.obj');
        expect(cmp.__getHelpersForTesting().isExpanded(node), 'starts collapsed').toBe(false);
        withCtxClipboard({ writeText }, () => cmp.onRowDblClick(new MouseEvent('dblclick'), node));
        await Promise.resolve();
        await Promise.resolve();
        expect(cmp.__getHelpersForTesting().isExpanded(node), 'expanded after dblclick').toBe(true);
        expect(writeText).not.toHaveBeenCalled();
      });
      ...
      it('collapses an expanded container on dblclick', async () => {
        ...
        const writeText = vi.fn().mockResolvedValue(undefined);
        const node = nodeAt('$.obj');
        expect(cmp.__getHelpersForTesting().isExpanded(node), 'starts expanded').toBe(true);
        withCtxClipboard({ writeText }, () => cmp.onRowDblClick(new MouseEvent('dblclick'), node));
        await Promise.resolve();
        await Promise.resolve();
        expect(cmp.__getHelpersForTesting().isExpanded(node), 'collapsed after dblclick').toBe(
          false,
        );
        expect(writeText).not.toHaveBeenCalled();
      });
      ...
      it('toggles container on dblclick even when Alt is held (no copy)', async () => {
        ...
        await Promise.resolve();
        await Promise.resolve();
        expect(cmp.__getHelpersForTesting().isExpanded(node), 'Alt does not suppress toggle').toBe(
          true,
        );
        expect(writeText).not.toHaveBeenCalled();
      });
      ...
      it('copies the literal `{}` on dblclick of an empty object row', async () => {
        ...
        expect(
          cmp.__getHelpersForTesting().isExpanded(node),
          'expansion state unchanged on empty container',
        ).toBe(wasExpanded);
      ...
      it('skips when the dblclick target is an interactive descendant (kebab pill)', async () => {
        ...
        expect(kebab, 'expected an interactive data-row kebab button').toBeTruthy();
      ...
      it('skips when the dblclick target is the chevron toggle button (regression for issue #109)', async () => {
        ...
        expect(chevron, 'found the chevron toggle button on $.obj').toBeTruthy();
        const ev = new MouseEvent('dblclick', { bubbles: true });
    ...
    describe('rendering', () => {
      ...
      it('renders menu items with the right contextNode when the kebab is clicked via the DOM', async () => {
        ...
        expect(leafKebab, 'found a kebab on $.alpha').toBeTruthy();
        leafKebab!.click();
        fixture.detectChanges();
        ...
        expect(items.length, 'menu must render at least one item').toBeGreaterThan(0);
      ...
      it('renders Copy value as the first menu item, marked as the default action', async () => {
        ...
        expect(first.textContent?.trim().startsWith(cmp.ctxCopyValueLabel)).toBe(true);
    ...
    describe('surfaced default-shortcut row (Path Y)', () => {
      ...
      it('clicking the surfaced "Expand 1 level" row expands the clicked container', async () => {
        ...
        expect(
          cmp.__getHelpersForTesting().isExpanded(node),
          'expanded after surfaced shortcut click',
        ).toBe(true);
      });
      ...
      it('clicking the surfaced "Collapse from here" row collapses the clicked container', async () => {
        ...
        expect(
          cmp.__getHelpersForTesting().isExpanded(node),
          'collapsed after surfaced shortcut click',
        ).toBe(false);
      });
      ...
      it('renders a `.sr-only` a11y hint on the bolded surfaced row (collapsed container)', async () => {
        ...
        expect(surfaced, 'found surfaced bolded row').toBeTruthy();
      ...
      it('renders a `.sr-only` a11y hint on the bolded Copy value for primitives', async () => {
        ...
        expect(copyValue?.textContent?.trim().startsWith(cmp.ctxCopyValueLabel)).toBe(true);
    ...
    describe('Subtree submenu (Path Y + v0.19.4 elevation)', () => {
      it('renders the Subtree > trigger when at least 2 subtree actions apply', async () => {
        ...
        expect(trigger, 'Subtree trigger present').toBeTruthy();
        document.body
          .querySelectorAll('.cdk-overlay-backdrop')
          .forEach((b) => (b as HTMLElement).click());
        fixture.detectChanges();
      ...
      it('elevates a single Subtree item to row level instead of rendering Subtree >', async () => {
        ...
        expect(subtreeTrigger, 'Subtree submenu suppressed (collapseSame)').toBeUndefined();
        // Verify that the surfaced shortcut row carries the same
        // Collapse action (so the user still has access).
        const surfaced = items.find((m) => m.classList.contains('ctx-default-action'));
        expect(surfaced?.textContent?.trim().startsWith(cmp.ctxCollapseFromHereLabel)).toBe(true);
        document.body
          .querySelectorAll('.cdk-overlay-backdrop')
          .forEach((b) => (b as HTMLElement).click());
        fixture.detectChanges();
      ...
      it('hides the Subtree > trigger on a primitive row (no subtree to act on)', async () => {
        ...
        expect(trigger, 'Subtree trigger absent on primitive').toBeUndefined();
        document.body
          .querySelectorAll('.cdk-overlay-backdrop')
          .forEach((b) => (b as HTMLElement).click());
        fixture.detectChanges();
      ...
      it('Subtree -> Expand submenu trigger renders when any depth predicate applies', async () => {
        ...
        expect(cmp.showExpandFromHereMenu(node)).toBe(true);
      });
    });
    ...
    describe('single-item elevation (v0.19.4)', () => {
      it('expandFromHereSingleAction returns null when Expand from here has zero depth options', async () => {
        ...
        expect(
          cmp.showExpandFromHereSubmenu(outer),
          'Expand sub-submenu hides when zero depths reachable',
        ).toBe(false);
      });
      ...
      it('expandFromHereSingleAction returns null when 2+ items are visible', async () => {
        ...
        expect(
          cmp.showExpandFromHereSubmenu(outer),
          'Expand sub-submenu renders for 2+ items',
        ).toBe(true);
      });
      ...
      it('subtreeElevatedAction returns null when 2+ Subtree items apply', async () => {
        ...
        expect(cmp.showSubtreeMenu(a)).toBe(true);
      });
      ...
      it('subtreeElevatedAction returns collapseSame when surfaced shortcut would duplicate', async () => {
        ...
        expect(
          cmp.showSubtreeMenu(obj),
          'Subtree submenu suppressed when surfaced row duplicates',
        ).toBe(false);
      });
      ...
      it('subtreeElevatedAction returns null for collapsed container with only primitives (v0.23.0)', async () => {
        ...
        expect(
          cmp.showExpandAllFromHere(obj),
          'Expand all from here remains conceptually reachable',
        ).toBe(true);
        expect(
          cmp.hasContainerDescendants(obj),
      ...
      it('top-level Expand all row renders when subtree has container descendants', async () => {
        ...
        cmp.contextNode.set(outer);
        expect(
          cmp.showExpandAllFromHere(outer),
          'showExpandAllFromHere true: subtree has collapsed container descendant',
        ).toBe(true);
        expect(
          cmp.hasContainerDescendants(outer),
          'hasContainerDescendants true: mid is a container descendant',
        ).toBe(true);
      });
      ...
      it('top-level Expand all row hides for primitives-only collapsed containers', async () => {
        ...
        expect(
          cmp.showExpandAllFromHere(outer),
          'Expand all still conceptually meaningful (outer is collapsed)',
        ).toBe(true);
        expect(
          cmp.hasContainerDescendants(outer),
      ...
      it('top-level Expand all row hides for primitive leaves and empty containers', async () => {
        ...
        expect(cmp.showExpandAllFromHere(alpha)).toBe(false);
        expect(cmp.showExpandAllFromHere(empty)).toBe(false);
      });
      ...
      it('top-level Expand all row hides when subtree is fully expanded', async () => {
        ...
        expect(
          cmp.showExpandAllFromHere(outer),
          'Nothing collapsed downstream -> nothing to expand',
        ).toBe(false);
      });
      ...
      it('Expand all label returns base form for primitives-only containers (safe default)', async () => {
        ...
        expect(
          cmp.hasContainerDescendants(outer),
          'precondition: primitives-only -> row hidden in production',
        ).toBe(false);
      ...
      it('top-level Expand all row is not bolded (dblclick-mirror mandate guardrail)', async () => {
        ...
        expect(cmp.showExpandAllFromHere(outer), 'precondition: row must be visible').toBe(true);
        expect(cmp.hasContainerDescendants(outer), 'precondition: row must be visible').toBe(true);
        ...
        expect(expandAllRow, 'Top-level Expand all from here row must render').toBeTruthy();
        expect(
          expandAllRow!.classList.contains('ctx-default-action'),
          'Top-level Expand all row must NOT carry .ctx-default-action',
        ).toBe(false);
        document.body
          .querySelectorAll('.cdk-overlay-backdrop')
          .forEach((b) => (b as HTMLElement).click());
      ...
      it('Subtree > Expand sub-submenu no longer contains an All leaf', async () => {
        ...
        cmp.contextNode.set(outer);
        expect(cmp.expandFromHereSingleAction(outer)).toBeNull();
        expect(cmp.showExpandFromHereMenu(outer)).toBe(false);
        expect(cmp.hasContainerDescendants(outer)).toBe(false);
      });
      ...
      it('lone-+1 in-Subtree elevation is suppressed under expandRow defaults (no duplication with surfaced row)', async () => {
        ...
        expect(cmp.hasContainerDescendants(outer), 'mid is a container descendant').toBe(true);
        // Predicate fires -> Expand-from-here disappears entirely
        // (no items, no single elevation, submenu hides).
        expect(cmp.expandFromHereSingleAction(outer)).toBeNull();
        expect(cmp.showExpandFromHereSubmenu(outer)).toBe(false);
        expect(cmp.showExpandFromHereMenu(outer)).toBe(false);
      });
      ...
      it('lone-+N in-Subtree elevation still fires for non-expandRow defaults', async () => {
        ...
        fixture.detectChanges();
        cmp.contextNode.set(outer);
        expect(cmp.defaultActionKind(), 'Outer is expanded -> default is collapseRow').toBe(
          'collapseRow',
        );
        ...
        expect(cmp.showExpandFromHereSubmenu(outer)).toBe(false);
        expect(cmp.showExpandFromHereMenu(outer)).toBe(true);
      ...
      it('Expand from here +6..+9 are visible on deep enough subtrees', async () => {
        ...
        for (let n = 1; n <= 9; n++) {
          expect(
            cmp.showExpandToDepth(root, n),
            `+${n} should be visible on a 9-deep subtree`,
          ).toBe(true);
        }
      });
      ...
      it('Expand from here +6..+9 are hidden on shallow subtrees', async () => {
        ...
        expect(cmp.showExpandToDepth(outer, 2), '+2 visible (maxDescendantDepth >= 2)').toBe(true);
        expect(cmp.showExpandToDepth(outer, 3), '+3 hidden (maxDescendantDepth < 3)').toBe(false);
        for (let n = 3; n <= 9; n++) {
          expect(cmp.showExpandToDepth(outer, n), `+${n} must hide on a 2-deep subtree`).toBe(
            false,
          );
        }
      });
      ...
      it('expandSingleElevatedLabel returns depth-N elevated label for depth 1..9', async () => {
        ...
        for (let depth = 1; depth <= 9; depth++) {
          ...
          expect(label, `depth=${depth}`).toBeTruthy();
        }
      });
    });
    ...
    describe('icons (Phase 3)', () => {
      it('renders a leading <jj-icon> on every top-level menu item', async () => {
        ...
        for (const item of items) {
          expect(
            item.querySelector('jj-icon'),
            `menu item "${(item.textContent ?? '').trim()}" has a leading jj-icon`,
          ).toBeTruthy();
        }
        document.body
          .querySelectorAll('.cdk-overlay-backdrop')
      ...
      it('renders a leading <jj-icon> on every top-level menu item for a container row', async () => {
        ...
        expect(
          cmp.showExpandAllFromHere(outer),
          'precondition: Expand-all top-level row must be visible',
        ).toBe(true);
        expect(
          cmp.hasContainerDescendants(outer),
          'precondition: Expand-all top-level row must be visible',
        ...
        expect(
          expandAllRow,
          'precondition: Expand-all top-level row must be in the rendered menu',
        ).toBeTruthy();
        for (const item of items) {
          expect(
            item.querySelector('jj-icon'),
  ...
  describe('dblclick container toggle (issue #109, DOM-level)', () => {
    ...
    function objRow(): HTMLElement {
      ...
      expect(row, 'found the $.obj container row').toBeTruthy();
      return row!;
    }
    ...
    function objChevron(): HTMLButtonElement {
      ...
      expect(chevron, 'found the $.obj chevron toggle').toBeTruthy();
      return chevron!;
    }
    ...
    it('real dblclick on container row toggles expansion and does not copy', async () => {
      ...
      expect(cmp.__getHelpersForTesting().isExpanded(node), 'expanded after real dblclick').toBe(
        true,
      );
      expect(writeText).not.toHaveBeenCalled();
      expect(logger.info).toHaveBeenCalledWith('tree.row.doubleClickToggle', {
      ...
      expect(
        cmp.__getHelpersForTesting().isExpanded(node),
        'collapsed after second real dblclick',
      ).toBe(false);
      expect(writeText).not.toHaveBeenCalled();
      expect(logger.info).toHaveBeenCalledWith('tree.row.doubleClickToggle', {
        action: 'collapse',
    ...
    it('dblclick on the chevron does not invoke row dblclick behavior', async () => {
      ...
      expect(logger.info).not.toHaveBeenCalledWith('tree.row.doubleClickToggle', expect.anything());
    });
  });
  ...
  describe('Alt-modifier copy escape', () => {
    ...
    async function createAltCopyFixture(): Promise<Mocked<LoggerService>> {
    ...
    function greetingRow(): HTMLElement {
      ...
      expect(row, 'found the $.greeting tree row').toBeTruthy();
      return row!;
    }
    ...
    async function openGreetingContextMenu(): Promise<HTMLButtonElement> {
      ...
      expect(button, 'found the Copy value context-menu item').toBeTruthy();
      return button!;
    }
  ...
  describe('keyboard copy (Ctrl+C / Cmd+C, focused tree row)', () => {
    ...
    it('Ctrl+C on a focused container row copies pretty JSON without changing expansion', async () => {
      ...
      expect(
        cmp.__getHelpersForTesting().isExpanded(node),
        'expansion state unchanged by keyboard copy',
      ).toBe(wasExpanded);
    });
    ...
    it('Ctrl+C on a focused expanded container row also copies pretty JSON without collapsing', async () => {
      ...
      expect(
        cmp.__getHelpersForTesting().isExpanded(node),
        'still expanded after keyboard copy',
      ).toBe(true);
    });
    ...
    it('Ctrl+Shift+C is a no-op (does not steal devtools shortcut)', async () => {
      ...
      await Promise.resolve();
      expect(writeText).not.toHaveBeenCalled();
      expect(prevented, 'Ctrl+Shift+C must not be preventDefaulted').toBe(false);
    });
    ...
    it('Ctrl+Alt+C is a no-op (does not steal AltGr layouts)', async () => {
      ...
      await Promise.resolve();
      expect(writeText).not.toHaveBeenCalled();
      expect(prevented, 'Ctrl+Alt+C must not be preventDefaulted').toBe(false);
    });
    ...
    it('emits tree.keyboard.copyValue with escaped: false', async () => {
      ...
      expect(logger.info).not.toHaveBeenCalledWith('tree.contextMenu.copyValue', expect.anything());
    });
    ...
    it('does not fire when the keydown originates from a descendant of the row', async () => {
      ...
      expect(chevron, 'found the $.obj chevron button').toBeTruthy();
      ...
      await Promise.resolve();
      await Promise.resolve();
      expect(writeText, 'descendant bubble must not trigger row-level copy').not.toHaveBeenCalled();
      expect(prevented, 'descendant bubble must not be preventDefaulted').toBe(false);
    });
    ...
    it('Ctrl+C on a real focused container row copies its value (DOM-level)', async () => {
      ...
      try {
        ...
        expect(prevented, 'Ctrl+C should be preventDefaulted').toBe(true);
      } finally {
        document.body.removeChild(fixture.nativeElement);
      }
    });
