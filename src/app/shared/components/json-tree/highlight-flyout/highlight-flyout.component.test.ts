...
describe('HighlightFlyoutComponent', () => {
  ...
  describe('focusEntry', () => {
    ...
    it('flips the kbd-focused class onto the active cell', async () => {
      ...
      expect(preferredBar().classList.contains('kbd-focused')).toBe(true);
    });
  });
  ...
  describe('keyboard navigation', () => {
    ...
    it('ArrowDown from row 2 clamps and re-asserts keyboard ring', async () => {
      ...
      expect(document.activeElement).toBe(list[5]!);
      expect(
        list[5]!.classList.contains('kbd-focused'),
        'clamp keypress re-asserts kbd-focused on the active cell',
      ).toBe(true);
    });
    ...
    it('ArrowUp from Preferred clamps and re-asserts keyboard ring', async () => {
      ...
      expect(document.activeElement).toBe(preferredBar());
      expect(
        preferredBar().classList.contains('kbd-focused'),
        'clamp keypress re-asserts kbd-focused on Preferred',
      ).toBe(true);
    });
    ...
    it('ArrowLeft clamps at column 0 and re-asserts keyboard ring', async () => {
      ...
      expect(document.activeElement).toBe(list[0]!);
      expect(
        list[0]!.classList.contains('kbd-focused'),
        'clamp keypress re-asserts kbd-focused on the active cell',
      ).toBe(true);
    });
    ...
    it('ArrowRight clamps at column 4 and re-asserts keyboard ring', async () => {
      ...
      expect(
        list[4]!.classList.contains('kbd-focused'),
        'clamp keypress re-asserts kbd-focused on the active cell',
      ).toBe(true);
    });
  ...
  describe('focus ring modality', () => {
    it('keeps kbd-focused only on the active cell, not idle cells', async () => {
      ...
      const list = swatches();
      expect(list[0]!.classList.contains('kbd-focused')).toBe(true);
      expect(preferredBar().classList.contains('kbd-focused')).toBe(false);
      expect(list[1]!.classList.contains('kbd-focused')).toBe(false);
    });
    ...
    it('drops kbd-focused after a mouse hover (interactionMode flips to mouse)', async () => {
      ...
      expect(list[3]!.classList.contains('kbd-focused')).toBe(false);
      expect(preferredBar().classList.contains('kbd-focused')).toBe(false);
    });
    ...
    it('clamp keypress after mouse hover re-asserts the keyboard ring (col 4 boundary)', async () => {
      ...
      const list = swatches();
      expect(document.activeElement).toBe(list[4]!);
      expect(list[4]!.classList.contains('kbd-focused')).toBe(true);

      // Mouse-hover the same cell drops the ring.
      list[4]!.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
      fixture.detectChanges();
      expect(list[4]!.classList.contains('kbd-focused')).toBe(false);
      ...
      expect(
        list[4]!.classList.contains('kbd-focused'),
        'clamp at col 4 after mouse hover must re-assert kbd-focused',
      ).toBe(true);
    });
    ...
    it('clamp keypress after mouse hover re-asserts the keyboard ring (col 0 boundary)', async () => {
      ...
      fixture.detectChanges();
      expect(list[0]!.classList.contains('kbd-focused')).toBe(false);
      ...
      expect(
        list[0]!.classList.contains('kbd-focused'),
        'clamp at col 0 after mouse hover must re-assert kbd-focused',
      ).toBe(true);
    });
    ...
    it('clamp keypress after mouse hover re-asserts the keyboard ring (row 2 boundary)', async () => {
      ...
      list[5]!.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
      fixture.detectChanges();
      expect(list[5]!.classList.contains('kbd-focused')).toBe(false);
      ...
      expect(
        list[5]!.classList.contains('kbd-focused'),
        'clamp at row 2 after mouse hover must re-assert kbd-focused',
      ).toBe(true);
    });
    ...
    it('clamp keypress after mouse hover re-asserts the keyboard ring (Preferred boundary)', async () => {
      ...
      expect(
        preferredBar().classList.contains('kbd-focused'),
        'clamp on Preferred after mouse hover must re-assert kbd-focused',
      ).toBe(true);
    });
    ...
    it('held arrow key at a clamp boundary does not emit apply or duplicate ring transitions', async () => {
      ...
      expect(applyEvents.length, 'clamps must never emit apply').toBe(0);
      expect(document.activeElement).toBe(list[4]!);
      expect(list[4]!.classList.contains('kbd-focused')).toBe(true);
    });
  });
