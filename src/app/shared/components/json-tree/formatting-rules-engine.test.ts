    ...
    it('matches only when both key and value text conditions match for every matchType combo', () => {
      for (const keyCase of textMatchCases) {
        for (const valueCase of textMatchCases) {
          ...
          expect(
            matchingResult.keyStyle.bold,
            `${keyCase.matchType} key and ${valueCase.matchType} value should match`,
          ).toBe(true);
          ...
          expect(keyMissResult, `${keyCase.matchType} key miss should skip pair rule`).toBe(
            EMPTY_RULE_RESULT,
          );
          ...
          expect(valueMissResult, `${valueCase.matchType} value miss should skip pair rule`).toBe(
            EMPTY_RULE_RESULT,
          );
        }
      }
  ...
  describe('pair rules with predicate value matches', () => {
    ...
    it('matches has_content and lacks_content according to the content truth table', () => {
      for (const contentPredicateCase of contentPredicateCases) {
        ...
        expect(lacksContentMatched, `${contentPredicateCase.label} lacks_content result`).toBe(
          contentPredicateCase.lacksContent,
        );
        expect(hasContentMatched, `${contentPredicateCase.label} has_content result`).toBe(
          contentPredicateCase.hasContent,
        );
      }
    });
    ...
    it('keeps has_content and lacks_content mutually exclusive', () => {
      for (const contentPredicateCase of contentPredicateCases) {
        ...
        expect(
          lacksContentMatched && hasContentMatched,
          `${contentPredicateCase.label} should not match both content predicates`,
        ).toBe(false);
      }
    });
    ...
    it('accepts has_content and lacks_content through the predicate validator gate', () => {
      ...
      for (const validPredicateCase of validPredicateCases) {
        ...
        expect(
          result.matchedRules,
          `${validPredicateCase.predicate} should pass isValidPredicate`,
        ).toEqual([
          {
            setId: 's1',
            ruleId: 'pair-r1',
    ...
    it('implements every is_not predicate as the opposite truth value', () => {
      ...
      for (const predicateCase of predicateCases) {
        expect(
          predicateResult(predicateCase.positive, predicateCase.matchingNode).keyStyle.bold,
          `${predicateCase.positive} should match its positive node`,
        ).toBe(true);
        expect(
          predicateResult(predicateCase.positive, predicateCase.nonMatchingNode),
          `${predicateCase.positive} should miss its negative node`,
        ).toBe(EMPTY_RULE_RESULT);
        expect(
          predicateResult(predicateCase.negative, predicateCase.matchingNode),
          `${predicateCase.negative} should miss the positive node`,
        ).toBe(EMPTY_RULE_RESULT);
        expect(
          predicateResult(predicateCase.negative, predicateCase.nonMatchingNode).keyStyle.bold,
          `${predicateCase.negative} should match the negative node`,
        ).toBe(true);
      }
    });
  });
