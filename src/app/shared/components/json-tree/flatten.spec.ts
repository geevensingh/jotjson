...
describe('flatten', () => {
  it('returns a single leaf for a primitive root', () => {
    ...
    expect(items[0]).toEqual(
      jasmine.objectContaining({ kind: 'leaf', level: 0, expandable: false }),
    );
  ...
  it('renders an unexpanded non-empty object as an open row only (children hidden)', () => {
    ...
    expect(items).toHaveSize(1);
    expect(items[0]).toEqual(
      jasmine.objectContaining({ kind: 'open', level: 0, expandable: true }),
    );
  });
  ...
  it('emits no leaf row when a non-empty container is in expandedPaths but has children expanded', () => {
    ...
    const expanded = expandedFor(root, () => true);
    const items = flattenTree(root, expanded);
    expect(items.filter((i) => i.kind === 'open')).toHaveSize(2); // $ and $.a
    expect(items.filter((i) => i.kind === 'close')).toHaveSize(2);
    expect(items.filter((i) => i.kind === 'leaf')).toHaveSize(2); // $.a[0] and $.a[1]
  });
