...
describe('IconComponent', () => {
  ...
  for (const name of allIcons) {
    it(`renders an SVG with at least one shape for "${name}"`, async () => {
      ...
      expect(shapeCount, `${name} should render at least one shape`).toBeGreaterThan(0);
    });
  }
  ...
  it('defaults to size 20 without auto sizing', async () => {
    ...
    expect(hostElement.classList.contains('jj-icon--auto')).toBe(false);
  });
  ...
  it('omits SVG dimensions and marks the host in auto mode', async () => {
    ...
    expect(hostElement.classList.contains('jj-icon--auto')).toBe(true);
  });
