...
describe('RuleEditorComponent (M6d-2 autosave)', () => {
  it('hydrates from cache and starts in idle (not dirty)', () => {
    ...
    expect(ctx.fixture.componentInstance.isDirty()).toBe(false);
  ...
  describe('mutators', () => {
    it('setName updates editable', () => {
      ...
      expect(ctx.fixture.componentInstance.isDirty()).toBe(true);
    });
  ...
  describe('validity', () => {
    it('flags empty name', () => {
      ...
      if (v.kind === 'invalid') {
        expect(v.reasons.some((r) => r.includes('Name'))).toBe(true);
      }
    });
    ...
    it('flags bad hex', () => {
      ...
      expect(v.kind).toBe('invalid');
      if (v.kind === 'invalid') {
        expect(v.reasons.some((r) => r.includes('color'))).toBe(true);
      }
    });
    ...
    it('addRule on a freshly-loaded set leaves validity invalid with matchValueEmpty reason', () => {
      ...
      expect(v.kind).toBe('invalid');
      if (v.kind === 'invalid') {
        expect(v.reasons.some((r) => r.includes('missing a match value'))).toBe(true);
      }
    });
    ...
    it('treats unknown future rule kinds as invalid without throwing', () => {
      ...
      expect(v.kind).toBe('invalid');
      if (v.kind === 'invalid') {
        expect(v.reasons.some((reason) => reason.includes('unsupported format'))).toBe(true);
      }
    });
  });
