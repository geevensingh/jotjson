...
function setup(opts: SetupOpts = {}) {
  ...
  const stub = {
    ...
    update: vi.fn().mockReturnValue(of(ruleSet())),
    delete: vi.fn().mockReturnValue(of(void 0)),
    get: vi.fn().mockReturnValue(of(ruleSet())),
    refresh: vi.fn(),
    toggleActive: vi.fn().mockImplementation((id: string) => {
  ...
  const dialog = {
    open: vi.fn().mockReturnValue(dialogRefStub),
  };
...
describe('FormattingRulesComponent (M6d-1 baseline)', () => {
  ...
  it('creates a new rule set and navigates into the editor', async () => {
    ...
    expect(stub.create).toHaveBeenCalledWith(expect.objectContaining({ rules: [] }));
...
describe('FormattingRulesComponent - M6e card actions', () => {
  ...
  it('rename happy path: pencil click -> Enter -> update + snack', async () => {
    ...
    stub.update.mockReturnValue(of(ruleSet({ id: 'a', name: 'Renamed', version: 4 })));
    await settle(fixture);
    ...
    expect(stub.update).toHaveBeenCalledWith('a', expect.objectContaining({ name: 'Renamed' }), 3);
    expect(snack.open).toHaveBeenCalled();
    fixture.detectChanges();
  ...
  it('rename 412: refetches, repopulates input, stays in rename mode with conflict message', async () => {
    ...
    stub.get.mockReturnValue(of(ruleSet({ id: 'a', name: 'New Name From Server', version: 4 })));
    await settle(fixture);
  ...
  it('delete cancel (dialog returns false) does not call delete', async () => {
    ...
    dialogRefStub.afterClosed.mockReturnValue(of(false));
    await settle(fixture);

    await fixture.componentInstance.deleteSet(sets[0]);
    expect(dialog.open).toHaveBeenCalledWith(ConfirmDialogComponent, expect.any(Object));
  ...
  it('delete confirmed calls service.delete', async () => {
    ...
    dialogRefStub.afterClosed.mockReturnValue(of(true));
    await settle(fixture);
  ...
  it('clone-preset dialog success navigates into the editor', async () => {
    ...
    dialogRefStub.afterClosed.mockReturnValue(
    ...
    expect(dialog.open).toHaveBeenCalledWith(ClonePresetDialogComponent, expect.any(Object));
  ...
  it('per-row busy gate: while delete is in flight, that row is disabled but others are not', async () => {
    ...
    stub.delete.mockReturnValue(deleteSubject.asObservable());
    await settle(fixture);
