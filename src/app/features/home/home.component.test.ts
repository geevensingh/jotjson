...
describe('HomeComponent (unit-level)', () => {
  ...
  it('onCopy writes the editor text to the clipboard', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    fixture.componentInstance.content.set('{"a":1}');
    const spy = vi.spyOn(navigator.clipboard, 'writeText').mockReturnValue(Promise.resolve());
  ...
  it('onPaste auto-unescapes an escaped JSON payload and formats it', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const escaped = '{\\r\\n    \\"a\\": 1,\\r\\n    \\"b\\": 2\\r\\n }';
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve(escaped));
  ...
  it('onPaste leaves already-valid JSON unchanged (no unescape)', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const text = '{"a":1}';
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve(text));
    await fixture.componentInstance.onPaste();
    await waitForDoubleAnimationFrame();
  ...
  it('onPaste emits paste.handle telemetry for valid JSON', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        sizeBytes,
        clipboardReadMs: expect.any(Number),
        parseMs: expect.any(Number),
        syncHandlerMs: expect.any(Number),
        firstPaintMs: expect.any(Number),
      }),
    );
  });
  ...
  it('onPaste does not emit paste.handle for whitespace-only clipboard text', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const eventSpy = vi.spyOn(TestBed.inject(LoggerService), 'event');
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve('  \n\t  '));
  ...
  it('onEditorPaste emits paste.handle.editor with parseMs=0 when postPasteParses is true', async () => {
    ...
    expect(eventSpy.mock.calls.some((args) => args[0] === 'paste.handle')).toBe(false);
  });
  ...
  it('Ctrl+F is not routed to tree-search when the tree pane is hidden', () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const c = fixture.componentInstance;
    const focusSpy = vi.spyOn(c as unknown as { focusTreeSearch: () => void }, 'focusTreeSearch');
...
describe('HomeComponent narrow-viewport responsive layout (M7l)', () => {
  ...
  it('host element gets .tree-only class at narrow when persisted is "both"', () => {
    ...
    expect(split?.classList.contains('tree-only')).toBe(true);
    expect(split?.classList.contains('editor-only')).toBe(false);
  });
  ...
  it('host element honors persisted "editor-only" at narrow', () => {
    ...
    fixture.detectChanges();
    const split = fixture.nativeElement.querySelector('.split') as HTMLElement | null;
    expect(split?.classList.contains('editor-only')).toBe(true);
    expect(split?.classList.contains('tree-only')).toBe(false);
  });
  ...
  it('dispatchBeaconJump routes to tree (not the hidden editor) at narrow with persisted "both" + lastActive "editor"', () => {
    ...
    const tree = { selectByPathString: vi.fn() };
    vi.spyOn(c as unknown as { tree: () => unknown }, 'tree').mockReturnValue(tree);
  ...
  it('mid-drag move handler bails when narrow flips during drag', () => {
    ...
    vi.spyOn(splitHostEl!.nativeElement, 'getBoundingClientRect').mockReturnValue({
      top: 0,
      left: 0,
      right: 1000,
      bottom: 1000,
    ...
    expect(moveHandler, 'move handler should have been registered').not.toBeNull();
...
describe('cold-boot clipboard auto-paste', () => {
  ...
  async function createAskBannerHarness(text = '{"clipboard":true}'): Promise<ColdBootHarness> {
    ...
    expect(harness.component.coldBootClipboardBannerVisible()).toBe(true);
  ...
  function expectBannerCard(fixture: HomeFixture, visible: boolean): void {
    ...
    if (visible) {
      expect(bannerCard, 'cold-boot banner card should be visible').not.toBeNull();
    } else {
      expect(bannerCard, 'cold-boot banner card should be hidden').toBeNull();
    }
  }
  ...
  function emitColdBootChoice(fixture: HomeFixture, choice: ColdBootClipboardChoice): void {
    ...
    expect(bannerDebugElement, 'cold-boot banner host should exist').not.toBeNull();
  ...
  it('permission ungranted -> evaluator does nothing (no banner, no read, no hold)', async () => {
    ...
    expect(harness.component.coldBootClipboardBannerVisible()).toBe(false);
  ...
  it("preference 'ask' + valid object/array JSON -> banner visible; home.clipboard.coldBoot.prompt.shown emitted", async () => {
    ...
    expect(harness.eventSpy.mock.calls).toEqual([['home.clipboard.coldBoot.prompt.shown']]);
  });
  ...
  it('banner Just this time click -> replaces document, opens snackbar, emits prompt.choice, does NOT change preference', async () => {
    ...
    expect(harness.component.coldBootClipboardBannerVisible()).toBe(false);
    expectColdBootSnack(harness.snack);
    expect(harness.eventSpy.mock.calls).toEqual([
  ...
  it('snackbar Undo -> restores prior draft via document replacement; emits autoPaste.undo', async () => {
    ...
    expect(harness.component.content()).toBe('{"draft":true}');
    expect(harness.eventSpy.mock.calls).toEqual([['home.clipboard.coldBoot.autoPaste.undo']]);
  });
  ...
  it("snackbar reference retained -> verify the cold-boot snackbar's MatSnackBarRef is captured and reachable", async () => {
    ...
    expect(retainedRef, 'SnackBarRef should be retained after paste').not.toBeNull();
...
describe('HomeComponent sign-in restore (issue #84)', () => {
  ...
  it('writes the restore snapshot and calls signIn when sign-in is requested', () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const component = fixture.componentInstance;
    const signInSpy = vi.spyOn(TestBed.inject(AuthService), 'signIn');
...
describe('HomeComponent draft persistence skip (issue #84)', () => {
  ...
  it('writes draft content when no blob is loaded', () => {
    ...
    const setSpy = vi.spyOn(draftService, 'set');
    const fixture = TestBed.createComponent(HomeComponent);
...
describe('HomeComponent tree<->editor selection sync (issue #42)', () => {
  ...
  describe('issue #266 defer/restore (cursor-restore on settle)', () => {
    ...
    it('with sync OFF from the start, onCursorChange does not call selectByPathString', () => {
      ...
      component.onToggleSelectionSync();
      tree.selectByPathString.mockClear();
      tree.clearPendingSelectPath.mockClear();
...
describe('HomeComponent save() branching (M4a)', () => {
  ...
  function setup(opts: {
    ...
    const stub: StubBlobService = {
      ...
      get: vi.fn().mockReturnValue(of(blob())),
      events$: EMPTY,
    };
...
describe('HomeComponent manual highlights save flow (Phase 4)', () => {
  ...
  async function expectHighlightActionsVisible(
    ...
    expect(topPanel, 'row menu panel open').toBeTruthy();
    ...
    expect(
      topLabels.includes(tree.ctxHighlightLabel),
      `top-level Highlight visibility (visible=${visible})`,
    ).toBe(visible);
    expect(
      topLabels.includes(tree.ctxRemoveHighlightLabel),
      `top-level Remove highlight visibility (visible=${visible})`,
    ).toBe(visible);
    ...
    if (visible) {
      ...
      expect(subtreeTrigger, 'Subtree submenu trigger present').toBeTruthy();
      ...
      expect(subtreePanel, 'Subtree submenu panel open').toBeTruthy();
      const subtreeLabels = Array.from(
        ...
        .filter((text) => text.length > 0);
      expect(
        subtreeLabels.includes(tree.ctxHighlightTreeLabel),
        'subtree-scope Highlight (inside Subtree submenu) visibility',
      ).toBe(true);
      expect(
        subtreeLabels.includes(tree.ctxRemoveTreeHighlightLabel),
        'subtree-scope Remove highlight (inside Subtree submenu) visibility',
      ).toBe(true);
    }
  ...
  it('passes canEditHighlights false and hides highlight menu actions for anonymous viewers', async () => {
    ...
    const tree = getTree(fixture);
    expect(tree.canEditHighlights()).toBe(false);
    expect(
  ...
  it('passes canEditHighlights false and hides highlight menu actions for signed-in non-owners', async () => {
    ...
    expect(getTree(fixture).canEditHighlights()).toBe(false);
    await expectHighlightActionsVisible(fixture, false);
  });
  ...
  it('passes canEditHighlights true and shows highlight menu actions for the owner', async () => {
    ...
    expect(getTree(fixture).canEditHighlights()).toBe(true);
  ...
  it('keeps dirty true when highlights change while save is in flight', async () => {
    ...
    stub.update.mockReturnValue(updateSubject.asObservable());
    component.__loadBlobForTesting(blob({ highlights: [] }));
    component.onHighlightsChange([highlightFoo]);

    const savePromise = component.onSave();
    expect(component.saveInFlight()).toBe(true);
  ...
  it('shows a conflict toast when a 412 refetch event arrives during save', async () => {
    ...
    component.__loadBlobForTesting(blob({ highlights: [] }));
    component.onHighlightsChange([highlightFoo]);
    stub.update.mockImplementation(() => {
      eventsSubject.next({
        kind: 'conflict',
        id: 'blob-1',
        blob: blob({ version: 2 }),
  ...
  it('passes highlights into the tree and consumes highlight changes from the tree', () => {
    ...
    expect(tree.canEditHighlights()).toBe(true);
...
describe('HomeComponent document-title dirty indicator (issue #84)', () => {
  ...
  it('adds a star prefix to the dirty draft fallback title', () => {
    ...
    expect(mostRecentTitle(titleSpy).startsWith('* ')).toBe(true);
  });
  ...
  it('removes the star prefix after a successful save resets dirty state', async () => {
    ...
    const blobService = {
      create: vi.fn().mockReturnValue(of(updatedBlob)),
      update: vi.fn().mockReturnValue(of(updatedBlob)),
      get: vi.fn().mockReturnValue(of(originalBlob)),
      events$: EMPTY,
    };
...
describe('HomeComponent M7p extract-from-mixed-text', () => {
  ...
  it('toolbar paste with mixed text shows the extract banner', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const component = fixture.componentInstance;
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve('INFO log {"a":1}'));
    const extractor = TestBed.inject(JsonExtractorService);
    vi.spyOn(extractor, 'extractFromMixedText').mockReturnValue({
      text: '{ "a": 1 }',
      blockCount: 1,
      preservesComments: true,
      hasComments: false,
  ...
  it('toolbar paste with already-valid JSON does NOT show the extract banner', async () => {
    const fixture = TestBed.createComponent(HomeComponent);
    const component = fixture.componentInstance;
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve('{"a":1}'));
    const extractor = TestBed.inject(JsonExtractorService);
    const extractSpy = vi.spyOn(extractor, 'extractFromMixedText');
  ...
  it('native paste with mixed text fires extractor on pastedText only', () => {
    ...
    const component = fixture.componentInstance;
    const extractor = TestBed.inject(JsonExtractorService);
    const extractSpy = vi.spyOn(extractor, 'extractFromMixedText').mockReturnValue({
      text: '{ "a": 1 }',
      blockCount: 1,
      preservesComments: true,
      hasComments: false,
  ...
  it('M7u: editor paste of multiple blocks with prose extracts to a prose-preserving wrapper (real-world HTTP transcript)', () => {
    ...
    expect(candidate).not.toBeNull();
    expect(candidate?.data.blockCount).toBe(2);
    expect((candidate?.data.proseSegments ?? 0) >= 1).toBe(true);
    ...
    expect(keys.some((k) => k.startsWith('between_'))).toBe(true);
...
describe('HomeComponent extract-banner telemetry', () => {
  ...
  function setupTelemetryBed(): {
    ...
    const component = fixture.componentInstance;
    const extractor = TestBed.inject(JsonExtractorService);
    const extractorSpy = vi.spyOn(extractor, 'extractFromMixedText').mockReturnValue({
      text: '{ "a": 1 }',
      blockCount: 2,
  ...
  it('toolbar paste auto-focuses the banner Extract button', async () => {
    const focusSpy = vi.spyOn(ExtractJsonBannerComponent.prototype, 'focusExtractButton');
    const { component } = setupTelemetryBed();
    vi.spyOn(navigator.clipboard, 'readText').mockReturnValue(Promise.resolve('INFO log {"a":1}'));
...
describe('HomeComponent M7p title-suggester wiring', () => {
  ...
  it('onPaste clears any existing lastFilename', async () => {
    ...
    vi.spyOn(TestBed.inject(ClipboardPollingService), 'readForPaste').mockResolvedValue(
      '{"pasted":true}',
    );
    await fixture.componentInstance.onPaste();
    expect(fixture.componentInstance.lastFilename()).toBeNull();
...
describe('HomeComponent tree extract wiring (M7s)', () => {
  ...
  class FakeTreeStringExtractor {
    ...
    readonly beginGeneration = vi.fn().mockImplementation((): number => {
  ...
  function setup(): {
    ...
    const snack: ExtractSnackBarStub = {
      open: vi.fn().mockReturnValue(snackRefHarness.ref),
    };
    TestBed.configureTestingModule({
      imports: [HomeComponent],
      providers: [
  ...
  it('auto-expands the just-extracted node by exactly one level', () => {
    const expandSpy = vi.spyOn(JsonTreeComponent.prototype, 'expandNodeAtPath');
    const { component, treeExtractor } = setup();
    treeExtractor.setVersion(8);
    component.onValueChange('{"payload":"INFO {\\"a\\":1}","keep":true}');
  ...
  it('dismisses the live undo snackbar when ctrlZ-detected revert clears the pending state', () => {
    ...
    const snackRefHarness = createExtractSnackBarRefHarness();
    snack.open.mockReturnValue(snackRefHarness.ref);
  ...
  it('treats snackbar Undo as a no-op when ctrlZ already cleared the pending state', () => {
    ...
    expect(eventSpy).not.toHaveBeenCalledWith(
      'tree.extract.undo',
      expect.objectContaining({ source: 'snackbar' }),
    );
  });
...
describe('HomeComponent banner-extract undo (M7v)', () => {
  ...
  function setup(options: { editor: 'real' | 'failing' | 'absent' } = { editor: 'real' }): {
    ...
    const snack: ExtractSnackBarStub = {
      open: vi.fn().mockReturnValue(snackHarness.ref),
    };
    TestBed.configureTestingModule({
      imports: [HomeComponent],
      providers: [
    ...
    if (options.editor === 'absent') {
      ...
      editorStub = {
        applyEdit: vi.fn().mockReturnValue(false),
      };
  ...
  it('phantom-undo gate: a re-paste of priorText (replaceDocument path that bumps viewResetToken) does NOT emit home.extract.banner.undo', () => {
    ...
    expect(eventSpy).not.toHaveBeenCalledWith(
      ...
      expect.anything() as unknown as never,
    );
  });
...
describe('HomeComponent upload/format/minify/sort undo (issue #313)', () => {
  ...
  function createSnackHarness(): SnackHarness {
    const action = new Subject<void>();
    const dismissed = new Subject<unknown>();
    const dismissSpy = vi.fn().mockImplementation(() => dismissed.next(undefined));
  ...
  function createEditorStub(component: HomeComponent): ReplaceEditorStub {
    return {
      replaceAll: vi.fn().mockImplementation((text: string): ReplaceAllResult => {
  ...
  function setup(options: SetupOptions = {}): SetupResult {
    ...
    let harnessIndex = 0;
    const snackOpenSpy = vi.fn().mockImplementation(() => {
  ...
  it('upload + snackbar Undo restores text, lastFilename, highlights, mutatedPaths, suggestedTitles, extractedCandidate, and uploadError', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      'home.upload.undo',
      expect.objectContaining({ source: 'snackbar', trigger: 'pick' }),
      expect.objectContaining({ undoLatencyMs: expect.any(Number) }),
    );
  });
  ...
  it('upload + Ctrl+Z (re-emitted onValueChange) restores side-state with source=ctrlZ', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      'home.upload.undo',
      expect.objectContaining({ source: 'ctrlZ', trigger: 'pick' }),
      expect.objectContaining({ undoLatencyMs: expect.any(Number) }),
    );
  ...
  it('OS-launch upload + snackbar Undo emits home.upload.undo with trigger=osLaunch', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      'home.upload.undo',
      expect.objectContaining({ source: 'snackbar', trigger: 'osLaunch' }),
      expect.objectContaining({ undoLatencyMs: expect.any(Number) }),
    );
  });
  ...
  it('OS-launch upload + Ctrl+Z (re-emitted onValueChange) emits home.upload.undo with trigger=osLaunch', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      'home.upload.undo',
      expect.objectContaining({ source: 'ctrlZ', trigger: 'osLaunch' }),
      expect.objectContaining({ undoLatencyMs: expect.any(Number) }),
    );
    expect(snackHarnesses[0]!.dismissSpy).toHaveBeenCalled();
  });
  ...
  it('minify + snackbar Undo restores priorText and priorMode (jsonc -> json -> jsonc)', () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      'home.minify.undo',
      expect.objectContaining({ source: 'snackbar', priorMode: 'jsonc' }),
      expect.objectContaining({ undoLatencyMs: expect.any(Number) }),
    );
  });
  ...
  it('emits home.sort.applyFailed with reason=modelNull when replaceAll returns modelNull', () => {
    ...
    editorStub.replaceAll.mockReturnValue('modelNull');
  ...
  it('upload same-content-different-filename updates lastFilename without opening snackbar or warning', async () => {
    ...
    expect(editorStub.replaceAll).not.toHaveBeenCalled();
    expect(snackOpenSpy).not.toHaveBeenCalled();
    expect(warnSpy).not.toHaveBeenCalledWith('home.upload.applyFailed', expect.anything());
    expect(eventSpy).not.toHaveBeenCalledWith(
      'home.upload.undo',
      expect.anything(),
      expect.anything(),
  ...
  it('minify of already-minified jsonc flips mode and opens snackbar without warning (replaceAll inner noOp)', () => {
    ...
    expect(warnSpy).not.toHaveBeenCalledWith('home.minify.applyFailed', expect.anything());
  });
  ...
  it('emits home.format.applyFailed with reason=editsRejected when replaceAll returns editsRejected', () => {
    const { component, editorStub, warnSpy } = setup({ initialContent: '{"a":1,"b":2}' });
    editorStub.replaceAll.mockReturnValue('editsRejected');
