...
describe('RuleSetsToolbarComponent', () => {
  ...
  let snackStub: { open: jasmine.Spy };

  beforeEach(() => {
    localStorage.removeItem(PREFS_KEY);
    dialogStub = { open: jasmine.createSpy('dialog.open') };
    snackStub = { open: jasmine.createSpy('snack.open') };
  ...
  it('does NOT call list() when signed out (avoids 401 storms on home)', () => {
    const ruleSets = TestBed.inject(RuleSetsService);
    const spy = spyOn(ruleSets, 'list').and.returnValue(of([]));
    render();
  ...
  describe('signed in', () => {
    ...
    it('opens the clone-preset dialog when the trigger is clicked', () => {
      ...
      expect(dialogStub.open.calls.mostRecent().args[0]).toBe(ClonePresetDialogComponent);
    });
    ...
    it('auto-activates and toasts on successful clone', fakeAsync(() => {
      ...
      // Cache must contain the cloned set so setActive() does not filter it.
      setCache([cloned]);
      const setActiveSpy = spyOn(ruleSets, 'setActive').and.callThrough();
      ...
      expect(setActiveSpy).toHaveBeenCalledWith(['cloned-1']);
    ...
    it('does nothing when the user cancels the clone dialog', fakeAsync(() => {
      const ruleSets = TestBed.inject(RuleSetsService);
      setCache([]);
      const setActiveSpy = spyOn(ruleSets, 'setActive');
    ...
    it('does not crash when list() fails (the toolbar simply stays empty)', () => {
      const ruleSets = TestBed.inject(RuleSetsService);
      spyOn(ruleSets, 'list').and.returnValue(throwError(() => new Error('boom')));
