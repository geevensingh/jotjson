...
describe('HomeComponent (a11y shell landmarks)', () => {
  ...
  it('renders <main id="main-content"> as a focusable landmark', () => {
    ...
    expect(main)
      .withContext('home route must expose <main id="main-content"> for the skip-link target')
      .not.toBeNull();
  ...
  it('renders an <h1> inside <main> for screen-reader page identification', () => {
    ...
    expect(heading)
      .withContext('every route should have exactly one top-level heading inside <main>')
      .not.toBeNull();
    expect(heading?.textContent?.trim().length)
      .withContext('the <h1> must have non-empty content (visually-hidden via .sr-only is fine)')
      .toBeGreaterThan(0);
  });
});
